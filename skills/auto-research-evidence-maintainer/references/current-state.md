...
- Current source-only Table 3 state: one encrypted local `manual_review` candidate is bound to the active official package, exact PDF SHA, source page 5 and an audited bbox, and contains a 5×4 transcription. Verified-only CSV/XLSX, package-sidecar signing/audit and old-package compatibility contracts pass targeted tests. It has not been user-approved and the updated source is not installed yet.
- Remaining ordered gates: overwrite the same build54 candidate from the final clean source, complete real WebView screenshot-versus-grid review, approve/correct and export CSV/XLSX, then decide whether to issue an independently versioned official package carrying the verified sidecar; only after that run the final installed workflow and release gates.
