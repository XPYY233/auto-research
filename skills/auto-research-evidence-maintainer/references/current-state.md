...
- Source checkpoint: `7180df7` completes the separately observed workspace-source table display issue without cross-source identity mixing. Exact DOI/PDF SHA/page/screenshot SHA identity may expose an official verified grid read-only while preserving the workspace screenshot; the source batch passed 93 shared targets, 51 macOS targets and 15 release-contract checks. It is not yet in the installed build54 App.
