...
- Remaining ordered gates: keep the same build54, fix the separately observed workspace-source table display issue without cross-source identity mixing, then finish the remaining installed workflow and release gates. If the verified sidecar is distributed, issue and verify a new independently versioned official package rather than rewriting `1.1.0`.
