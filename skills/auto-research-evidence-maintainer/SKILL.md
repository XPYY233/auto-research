...
Before editing, read the top `PROJECT_HANDOFF.md` release section. The installed App is the single overwritable `1.2.0` build54 candidate at core commit `505d0c0`; build24 remains the verified stable rollback. Real AI, isolated extraction, and the official Table 3 source-image/PDF/return chain have passed. The source tree now also contains the verified-only official table review/export and signed-sidecar pipeline, and the App private state contains a PDF/page/bbox-bound 5×4 Table 3 `manual_review` candidate. This source has not yet been installed and the candidate has not been user-approved; do not describe it as shipped or scientifically verified. Keep all fixes on build54; do not create build55 until every installed-App user-flow gate passes. Verify installed identity and artifact hashes live because source completion, build completion, installation acceptance and scientific accuracy are separate claims.
