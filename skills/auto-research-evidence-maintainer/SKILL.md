...
Before editing, read the top `PROJECT_HANDOFF.md` release section. The installed App is the single overwritable `1.2.0` build54 candidate at core commit `b2578a6`; build24 remains the verified stable rollback. Real AI, isolated extraction, the official Table 3 source-image/PDF/return chain, and the PDF/page/bbox-bound 5×4 Table 3 review/restart/CSV/XLSX flow have passed in the installed App. The verified state lives in the independent encrypted review store; the immutable official `1.1.0` package was not rewritten. Keep all fixes on build54 and handle the separately queued workspace-table display issue before any stable claim. Do not create build55 until every installed-App user-flow gate passes. Verify installed identity and artifact hashes live because source completion, build completion, installation acceptance, package publication and scientific accuracy are separate claims.
