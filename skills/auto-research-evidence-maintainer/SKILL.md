...
Before editing, read the top `PROJECT_HANDOFF.md` release section. The installed App is the single overwritable `1.2.0` build54 candidate at core commit `505d0c0`; build24 remains the verified stable rollback. Real AI, isolated extraction, and the official Table 3 source-image/PDF/return chain have passed. The current table gap is missing human-verified row/column structure in the immutable package, which must not be guessed from an image. Keep all fixes on build54; do not create build55 until every installed-App user-flow gate passes. Verify installed identity and artifact hashes live because source completion, build completion, installation acceptance and scientific accuracy are separate claims.
