...
class ReleaseContractTests(unittest.TestCase):
    ...
    def test_document_tab_store_asset_cannot_be_omitted(self) -> None:
        value = json.loads((PROJECT_ROOT / "config" / "release-contract.json").read_text())
        value["web_assets"].pop(
            "src/auto_research/evidence/web/document_tab_store.js", None
        )
        with self.assertRaisesRegex(ReleaseContractError, "全部共享前端资产"):
            validate_release_contract(value)

    def test_pane_layout_controller_asset_cannot_be_omitted(self) -> None:
        value = json.loads((PROJECT_ROOT / "config" / "release-contract.json").read_text())
        value["web_assets"].pop(
            "src/auto_research/evidence/web/pane_layout_controller.js", None
        )
        with self.assertRaisesRegex(ReleaseContractError, "全部共享前端资产"):
            validate_release_contract(value)
    ...
    def test_fusion_pdf_controller_asset_cannot_be_omitted(self) -> None:
        value = json.loads((PROJECT_ROOT / "config" / "release-contract.json").read_text())
        value["web_assets"].pop(
            "src/auto_research/evidence/web/fusion_pdf_controller.js", None
        )
        with self.assertRaisesRegex(ReleaseContractError, "全部共享前端资产"):
            validate_release_contract(value)

    def test_ai_experience_assets_cannot_be_omitted(self) -> None:
        for relative in (
    ...
    def test_fusion_package_center_asset_is_hashed(self) -> None:
        contract = load_release_contract(PROJECT_ROOT / "config" / "release-contract.json")
        relative = "src/auto_research/evidence/web/fusion_package_center.js"
