...
(async()=>{{await controller.importOfficialPackage();assert.deepEqual(requests.map(x=>x[0]),['/api/desktop/evidence-packages/import','/api/desktop/evidence-packages','/api/desktop/package-center']);assert.equal(projected,1);assert.equal(state.lastOfficialResult.outcome,'activated');assert.equal(navigation,0);}})().catch(error=>{{console.error(error);process.exitCode=1}});
"""
