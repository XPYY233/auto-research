...
(async()=>{{
 ...
 api.state.evidence=[api.publicEvidence({{entity_type:'finding',item_id:5,paper_id:3,finding_text:'硬度随温度升高而下降',meaning:'温度效应',source_excerpt:'decreased with temperature',article_title:'Real paper',source_page:5}})];assert(await api.openEvidenceDetail(0,new El()));assert(secondary.innerHTML.includes('硬度随温度升高而下降'));closeActiveEvidence({{focus:false}});
