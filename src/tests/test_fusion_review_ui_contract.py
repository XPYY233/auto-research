...
class FusionReviewUIContractTests(unittest.TestCase):
    def test_findings_use_prose_layout_in_lists_and_details(self) -> None:
        source = (WEB / "fusion_review.js").read_text(encoding="utf-8")
        css = (WEB / "workbench.css").read_text(encoding="utf-8")
        self.assertGreaterEqual(source.count('row.type==="finding"?"结论"'), 2)
        self.assertIn('class="fusion-finding-summary"', source)
        self.assertIn('.fusion-result>b { max-width:44%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap; }', css)
...
"""
    ...
    def test_four_evidence_types_share_one_central_detail_workspace(self) -> None:
        ...
        self.assertIn('row.type==="finding"?"结论"', self.runtime)
        self.assertIn('class="fusion-finding-summary"', self.runtime)
        self.assertIn(".fusion-evidence-line { display:grid;width:100%;min-width:0;", self.css)
...
let calls=[];globalThis.fetch=(url,options={{}})=>{{calls.push([String(url),String(options.method||'GET')]);if(String(url)==='/api/desktop/ai/actions/literature_extraction/task-directory')return response({{schema_version:'literature-extraction-task-directory-v1',tasks:[],startup_recovery:{{recovered:0,already_completed:0,skipped_or_blocked:0}},issues:[]}});if(String(url)==='/api/desktop/ai/actions/literature_extraction/jobs')return response({{schema_version:'ai-execution-job-list-v1',scope:'literature_extraction',jobs:[completed]}});if(String(url)==='/api/desktop/review-queue')return response({{schema_version:'review-queue-v1',source_scope:'workspace',source_id:'workspace',paper_uid:null,total:0,items:[]}});if(String(url)==='/api/search-papers')return response([]);if(String(url)==='/api/desktop/ai/actions/literature_extraction/prepare')return response({{code:'literature_pdf_missing',message:'PDF 不可用',next_action:'replace_valid_pdf'}},false);throw new Error('unexpected '+url);}};
eval(fs.readFileSync({str(WEB / 'fusion_review.js')!r},'utf8'));const api=globalThis.AutoResearchFusion;
(async()=>{{
 api.state.view='search';api.state.paper={{id:7,title:'W-Ta',doi:'10.1/example'}};const result=await api.reconnectLiteratureJob();assert.equal(result.status,'completed');assert.equal(api.state.view,'search','background completion must not steal navigation');assert.equal(api.state.literatureReceipt.status,'completed');assert.deepEqual(calls,[['/api/desktop/ai/actions/literature_extraction/task-directory','GET'],['/api/desktop/ai/actions/literature_extraction/jobs','GET'],['/api/desktop/review-queue','GET'],['/api/search-papers','GET']],'reconnect adopts latest job without prepare consent or execute, then refreshes public views');
 api.state.paper={{id:8,title:'Other paper',doi:'10.1/other'}};api.state.literatureReceipt=null;calls=[];await api.reconnectLiteratureJob();assert.equal(api.state.literatureReceipt,null,'a receipt must not attach to another paper');assert.deepEqual(calls,[['/api/desktop/ai/actions/literature_extraction/jobs','GET'],['/api/desktop/review-queue','GET'],['/api/search-papers','GET']],'another paper completion also invalidates the review queue and catalog');
...
let calls=[];globalThis.fetch=(url,options={{}})=>{{calls.push([String(url),String(options.method||'GET'),String(options.body||'')]);if(String(url)==='/api/desktop/ai/actions/literature_extraction/recover-finalization')return response({{schema_version:'literature-extraction-recovery-result-v1',already_completed:false,completion:commit}});if(String(url)==='/api/desktop/review-queue')return response({{schema_version:'review-queue-v1',source_scope:'workspace',source_id:'workspace',paper_uid:null,total:0,items:[]}});if(String(url)==='/api/search-papers')return response([]);if(String(url)==='/api/desktop/ai/actions/literature_extraction/task-directory')return response(readyDirectory);throw new Error('unexpected '+url);}};
eval(fs.readFileSync({str(WEB / 'fusion_review.js')!r},'utf8'));const api=globalThis.AutoResearchFusion;
(async()=>{{
 ...
 assert.deepEqual(calls,[['/api/desktop/ai/actions/literature_extraction/recover-finalization','POST','{{"resume_token":"resume_abcdefghijklmnop"}}'],['/api/desktop/review-queue','GET',''],['/api/search-papers','GET',''],['/api/desktop/ai/actions/literature_extraction/task-directory','GET','']]);
...
"""
    ...
    def test_private_table_metadata_follows_active_visible_tab(self) -> None:
        program = f"""
(async()=>{{
const fs=require('fs'),assert=require('assert');
class Classes{{toggle(){{}}add(){{}}remove(){{}}contains(){{return false}}}}
class El{{constructor(){{this.hidden=false;this.dataset={{}};this.innerHTML='';this.textContent='';this.classList=new Classes();this.attrs={{}};this.scrollTop=0;}}setAttribute(k,v){{this.attrs[k]=String(v)}}removeAttribute(k){{delete this.attrs[k]}}querySelector(){{return null}}querySelectorAll(){{return []}}addEventListener(){{}}focus(){{throw Error('background table must not steal focus')}}}}
const ids={{}};for(const id of ['fusion-primary-editor-surface','fusion-secondary-editor-surface','fusion-secondary-editor-body','fusion-editor-group-content','fusion-editor-group-separator','fusion-inspector-title','fusion-inspector-body','fusion-evidence-detail-body','fusion-detail-heading','fusion-detail-identity'])ids['#'+id]=new El();
...
globalThis.innerWidth=1280;globalThis.localStorage={{getItem:()=>null,setItem(){{}}}};
for(const file of ['document_tab_store.js','pane_layout_controller.js','workspace_layout_controller.js','fusion_review.js'])eval(fs.readFileSync({str(WEB)!r}+'/'+file,'utf8'));
const api=globalThis.AutoResearchFusion,tabs=api.documentTabs,body=ids['#fusion-inspector-body'];api.state.view='search';api.state.searchMode='precise';
const row=id=>({{type:'table',sourceScope:'private',sourceId:'private-lab',entityUid:id,title:'表 '+id}});
const dto=(id,page=1)=>({{schema_version:'personal-table-page-v1',source_id:'private-lab',entity_uid:id,title:'真实表 '+id,sheet_name:'数据',columns:[{{name:'dose',role:'independent',meaning:'剂量',unit:'dpa'}},{{name:'value',role:'dependent',meaning:'硬度',unit:'GPa'}},{{name:'error',role:'uncertainty',meaning:'不确定度',unit:'GPa'}}],conditions:{{温度:id==='A'?'300 K':'500 K'}},series:[{{name:'系列 '+id,x_column:'dose',y_column:'value',uncertainty_column:'error',description:'测量 <'+id+'>'}}],page,page_size:50,total:123,has_next:page<3,rows:[{{dose:'0',value:id==='A'?'4.1':'5.2',error:'0.2'}}]}});
const identity=id=>({{sourceScope:'private',sourceId:'private-lab',entityType:'table',entityUid:id}});
const pageA=api.publicPersonalTablePage(dto('A'),api.privateTableIdentity(row('A')),1);
const a=tabs.open({{tabId:'personal-table:A',kind:'personal-table',ownerView:'search',identity:identity('A'),title:'A',payload:{{row:row('A'),page:pageA,status:'ready'}}}},{{groupId:'primary',pin:true}});
api.state.evidenceDetailOpen=true;api.state.evidenceDetail=row('A');api.state.personalTablePage=pageA;
api.renderDocumentTabs();assert(body.innerHTML.includes('系列 A'));assert(body.innerHTML.includes('300 K'));
const calls=[],pending=new Map();globalThis.fetch=(url)=>{{calls.push(String(url));const uid=new URL(url,'http://local').searchParams.get('entity_uid');return new Promise(resolve=>pending.set(uid,raw=>resolve({{ok:true,headers:{{get:()=>null}},json:async()=>raw}})));}};
api.state.searchResults=[row('B'),row('C')];
const bLoad=api.openEvidenceDetail(0,null,{{preview:false,pin:true}}),b=tabs.activeTab('secondary');
assert(body.innerHTML.includes('正在读取'));assert(!body.innerHTML.includes('系列 A'),'loading B must not reuse old primary metadata');
const cLoad=api.openEvidenceDetail(1,null,{{preview:false,pin:true}}),c=tabs.activeTab('secondary');
pending.get('B')(dto('B'));assert(await bLoad);assert.equal(tabs.activeTab().tabId,c.tabId);assert(!body.innerHTML.includes('系列 B'),'late B must not replace C inspector');
pending.get('C')(dto('C'));assert(await cLoad);assert(body.innerHTML.includes('系列 C'));assert(!body.innerHTML.includes('系列 A'));assert(body.innerHTML.includes('不确定度 error'));assert(body.innerHTML.includes('&lt;C&gt;'));assert(ids['#fusion-secondary-editor-body'].innerHTML.includes('系列 C'),'real secondary mount includes the shared metadata');
assert(api.activateDocumentTab(b.tabId));assert(body.innerHTML.includes('系列 B'));assert(body.innerHTML.includes('500 K'));
const bReady=tabs.activeTab('secondary');for(const group of ['primary','secondary']){{const html=api.secondaryDocumentHTML(bReady,group);for(const text of ['系列 B','500 K','GPa','5.2','不确定度 error'])assert(html.includes(text),group+': '+text);assert(!html.includes('测量 <B>'),'metadata must be HTML escaped');}}
assert(api.activateDocumentTab(a.tabId));assert(body.innerHTML.includes('系列 A'));assert(!body.innerHTML.includes('系列 B'));assert.equal(api.state.view,'search');
api.state.view='settings';api.renderDocumentTabs();assert(!body.innerHTML.includes('系列 A'));assert.equal(body.dataset.privateTableTab,undefined);
api.state.view='search';tabs.activate(b.tabId);api.paneController.expandEditor('secondary',{{persist:false}});api.renderDocumentTabs();assert(body.innerHTML.includes('系列 B'));
api.paneController.collapseEditor('secondary',{{persist:false}});api.renderDocumentTabs();assert(!body.innerHTML.includes('系列 B'),'hidden group must not own inspector');assert.equal(body.dataset.privateTableTab,undefined);
api.paneController.expandEditor('secondary',{{persist:false}});tabs.update(b.tabId,{{payload:{{...bReady.payload,page:pageA}}}});api.renderDocumentTabs();assert(body.innerHTML.includes('暂时无法读取'));assert(!body.innerHTML.includes('系列 A'),'mismatched page identity must fail closed');
tabs.update(b.tabId,{{payload:{{...bReady.payload,page:null,status:'error'}}}});api.renderDocumentTabs();assert(body.innerHTML.includes('暂时无法读取'));assert(!body.innerHTML.includes('正在读取'));
assert.equal(calls.length,2,'tab/inspector projection does not refetch or call AI');assert(calls.every(url=>url.startsWith('/api/desktop/personal-experiments/table?')));
}})().catch(error=>{{console.error(error);process.exitCode=1}});
"""
        result = subprocess.run(["node", "-e", program], capture_output=True, text=True, check=False, timeout=8)
        self.assertEqual(result.returncode, 0, result.stderr)
...
eval(fs.readFileSync({str(WEB / 'document_tab_store.js')!r},'utf8'));eval(fs.readFileSync({str(WEB / 'fusion_review.js')!r},'utf8'));const api=globalThis.AutoResearchFusion,row={{type:'table',title:'纳米压痕原始表',sourceScope:'private',sourceId:'private-public',entityUid:'private:table:opaque'}};
...
api.documentTabs.open({{tabId:'personal-table:runtime',kind:'personal-table',ownerView:'search',title:row.title,identity:{{sourceScope:'private',sourceId:row.sourceId,entityType:'table',entityUid:row.entityUid}},payload:{{row,status:'loading'}}}},{{groupId:'primary',pin:true}});
...
const first=api.loadPrivateTablePage(row,1),second=api.loadPrivateTablePage(row,2);pending.get(2)(dto(2));await second;pending.get(1)(dto(1));await first;assert.equal(api.state.personalTablePage.page,2,'late page must not replace current page');assert(ids['#fusion-evidence-detail-body'].innerHTML.includes('硬度'));assert(ids['#fusion-evidence-detail-body'].innerHTML.includes('第 2 页'));assert(ids['#fusion-evidence-detail-body'].innerHTML.includes('300 K'));assert(ids['#fusion-evidence-detail-body'].innerHTML.includes('硬度曲线'));assert(ids['#fusion-inspector-body'].innerHTML.includes('辐照剂量'));assert(ids['#fusion-inspector-body'].innerHTML.includes('300 K'));assert(ids['#fusion-inspector-body'].innerHTML.includes('硬度曲线'));assert(calls.some(([url])=>url.includes('source_id=private-public')&&url.includes('entity_uid=private%3Atable%3Aopaque')&&url.includes('page_size=50')));
...
const workspaceUid='entity_table_'+'a'.repeat(32),unavailableUid='entity_table_'+'b'.repeat(32),staleUid='entity_table_'+'d'.repeat(32),workspaceURL=uid=>`/api/desktop/workspace-evidence?source_scope=workspace&source_id=workspace&entity_type=table&entity_uid=${{uid}}`,imageURL=uid=>`/api/desktop/workspace-evidence/image?source_scope=workspace&source_id=workspace&entity_type=table&entity_uid=${{uid}}`,pdfURL=uid=>`/api/desktop/workspace-evidence/pdf?source_scope=workspace&source_id=workspace&entity_type=table&entity_uid=${{uid}}#page=3`,calls=[];
const response=(body,ok=true)=>({{ok,headers:{{get:()=>null}},json:async()=>body}});globalThis.fetch=async url=>{{url=String(url);calls.push(url);const uid=new URL(url,'http://fusion.local').searchParams.get('entity_uid');if(url!==workspaceURL(uid))throw new Error('unexpected '+url);if(uid===staleUid)return response({{schema_version:'workspace-evidence-error-v1',code:'workspace_evidence_changed',message:'工作区证据已经变化，请重新检索。',retryable:true}},false);return response({{schema_version:'workspace-evidence-detail-v1',source_scope:'workspace',source_id:'workspace',entity_type:'table',entity_uid:uid,label:uid===workspaceUid?'Table 3':'Table unavailable',caption:'真实表格截图',source_page:3,source_excerpt:'Table 3 原文',bbox:[1,2,30,40],image_url:imageURL(uid),pdf_url:pdfURL(uid),asset_id:1358,paper_id:56,path:'/private/secret',content_hash:'f'.repeat(64),table_structure:uid===workspaceUid?{{schema_version:'workspace-table-grid-v1',available:true,status:'verified',origin:'linked_official',source_scope:'official',source_id:'official-v1',entity_type:'table',entity_uid:'entity_table_'+'c'.repeat(32),version:2,reason_codes:['exact_source_match'],rows:[['温度','硬度'],['300 K','4.63 GPa']]}}:{{schema_version:'workspace-table-grid-v1',available:false,status:'unavailable',reason_code:'workspace_table_structure_not_verified'}}}});}};
...
const row=(uid,title)=>({{type:'item',title,value:'4.1',unit:'GPa',meaning:title,sourceScope:'official',sourceId:'official-main',entityUid:uid,page:7,articleTitle:'论文 '+title,excerpt:'原文 '+title,quantities:[],variables:{{}},materials:[],conditions:'',methods:'',linkedItemCount:0,tags:[]}}),A=row('item:a','证据 A'),B=row('item:b','证据 B'),C=api.publicEvidence({{entity_type:'table',source_scope:'workspace',source_id:'workspace',entity_uid:workspaceUid,label:'Table 3',source_page:3,source_excerpt:'列表短摘录'}}),D=api.publicEvidence({{entity_type:'table',source_scope:'workspace',source_id:'workspace',entity_uid:unavailableUid,label:'Table unavailable',source_page:4}}),E=api.publicEvidence({{entity_type:'table',source_scope:'workspace',source_id:'workspace',entity_uid:staleUid,label:'Table stale',source_page:5}});assert(C&&D&&E);assert.equal(api.workspaceEvidenceIdentity(C).entityUid,workspaceUid);
api.state.view='search';api.state.searchMode='librarian';api.state.librarianResults=[A,B,C,D,E];api.state.librarianArticles=[];api.state.librarianResultsOpen=false;api.documentTabs.open({{tabId:'evidence:background',kind:'evidence',ownerView:'search',title:'已有详情',identity:{{sourceScope:'official',sourceId:'official-main',entityType:'item',entityUid:'item:background'}},payload:{{row:A,status:'ready'}}}},{{groupId:'secondary',pin:true}});api.paneController.expandEditor('secondary',{{persist:false}});api.paneController.expandSide('inspector',{{persist:false}});let geometry=api.coordinateTaskLayout('librarian-chat');assert.equal(geometry.inspectorDocked,false,'chat without selected evidence releases inspector');assert.equal(geometry.secondaryCollapsed,true,'librarian chat releases an unrelated secondary editor');assert.equal(api.paneLayout.editors.secondaryCollapsed,false,'task projection preserves the stored secondary preference');assert.equal(api.paneLayout.inspector.collapsed,false,'task suppression preserves inspector preference');api.renderLibrarianResultNavigator();assert.equal(api.setLibrarianResultsOpen(true),true);assert.equal(ids['#fusion-librarian-results'].hidden,false);assert(ids['#fusion-librarian-panel'].classList.contains('results-open'));geometry=api.applyPaneLayout();assert.equal(geometry.inspectorDocked,false,'result rail releases inspector space');assert.equal(geometry.contextDocked,false,'result rail releases unrelated context');assert.equal(geometry.secondaryCollapsed,true,'result rail releases unrelated secondary editor');assert.equal(api.paneLayout.editors.secondaryCollapsed,false,'result rail does not close the stored secondary group');
(async()=>{{assert(await api.openLibrarianEvidence(0));let secondary=api.documentTabs.activeTab('secondary');assert(secondary);geometry=api.applyPaneLayout();assert.equal(geometry.secondaryCollapsed,false,'evidence detail restores secondary editor');assert.equal(geometry.contextDocked,false,'1280 gives the librarian evidence detail the available secondary column');assert.equal(geometry.inspectorDocked,false,'librarian never restores a global inspector');assert.equal(api.togglePane('inspector'),false,'librarian rejects the global inspector');geometry=api.applyPaneLayout();assert.equal(geometry.inspectorDocked,false);assert.equal(secondary.payload.row.title,'证据 A');assert.equal(api.state.selectedEvidence.title,'证据 A');assert.equal(api.selectedEvidenceAIIdentity().entityUid,'item:a');assert.equal(globalThis.focused,evidenceQuestion,'opening a cited result restores focus inside its concrete evidence detail');
 ...
 assert(await api.openLibrarianEvidence(2));secondary=api.documentTabs.activeTab('secondary');assert.equal(secondary.identity.entityUid,workspaceUid);assert.equal(secondary.payload.row.entityUid,workspaceUid);assert.equal(secondary.payload.row.assetId,null);assert.equal(secondary.payload.row.paperId,null);assert.equal(api.selectedEvidenceAIIdentity().entityUid,workspaceUid);let detailHTML=api.secondaryDocumentHTML(secondary,'secondary');assert(detailHTML.includes('/api/desktop/workspace-evidence/image?'));assert(detailHTML.includes(workspaceUid));assert(detailHTML.includes('Table 3'));assert(detailHTML.includes('300 K'));assert(detailHTML.includes('4.63 GPa'));assert(detailHTML.includes('官方资料库已核验 · 同源只读'));assert(detailHTML.includes('结构 CSV'));assert(detailHTML.includes('查看原文'));const localVerified={{schemaVersion:'workspace-table-grid-v1',sourceScope:'workspace',sourceId:'workspace',entityUid:workspaceUid,version:1,status:'verified',reasonCodes:['reviewed'],rows:[['A','B']]}};assert(!api.tableStructureHTML('workspace-local',C,{{tableStructure:localVerified,tableStructureState:'ready'}},false).includes('结构 CSV'),'opaque local review must not expose an unsupported legacy export URL');const serialized=JSON.stringify(secondary.payload);for(const forbidden of ['/private/secret','asset_id','paper_id','content_hash','1358'])assert(!serialized.includes(forbidden),forbidden);assert.deepEqual(calls,[workspaceURL(workspaceUid)]);assert(!calls.some(url=>url.includes('/api/visual-assets/')||url.includes('/api/desktop/table-structures')));
 const expected=api.workspaceEvidenceIdentity(C),validRaw={{schema_version:'workspace-evidence-detail-v1',source_scope:'workspace',source_id:'workspace',entity_type:'table',entity_uid:workspaceUid,label:'Table 3',table_structure:{{schema_version:'workspace-table-grid-v1',available:false,status:'unavailable',reason_code:'workspace_table_structure_not_verified'}}}};assert.equal(api.publicWorkspaceEvidenceDetail({{...validRaw,source_id:'other'}},expected),null);assert.equal(api.publicWorkspaceEvidenceDetail({{...validRaw,entity_type:'figure'}},expected),null);assert.equal(api.publicWorkspaceEvidenceDetail({{...validRaw,entity_uid:unavailableUid}},expected),null);
 assert(await api.openLibrarianEvidence(3));secondary=api.documentTabs.activeTab('secondary');detailHTML=api.secondaryDocumentHTML(secondary,'secondary');assert(detailHTML.includes('/api/desktop/workspace-evidence/image?'));assert(detailHTML.includes(unavailableUid));assert(detailHTML.includes('尚无结构化行列'));assert.equal(secondary.payload.tableStructureState,'terminal');assert.equal(calls.at(-1),workspaceURL(unavailableUid));assert(!calls.some(url=>url.includes('/api/desktop/table-structures')));
 assert.equal(await api.openLibrarianEvidence(4),false,'stale workspace identity must fail closed');assert.equal(calls.at(-1),workspaceURL(staleUid));assert.equal(api.documentTabs.activeTab('secondary').payload.status,'error');
 for(const width of [900,720]){{globalThis.innerWidth=width;api.syncResponsivePaneLayout();assert.equal(api.setLibrarianResultsOpen(true),true);assert.equal(ids['#fusion-librarian-results'].hidden,false);assert.equal(ids['#fusion-librarian-results-toggle'].attrs['aria-expanded'],'true');assert.equal(api.setLibrarianResultsOpen(false),false);assert.equal(ids['#fusion-librarian-results'].hidden,true);}}
}})().catch(error=>{{console.error(error);process.exitCode=1;}});
"""
        result = subprocess.run(["node", "-e", program], capture_output=True, text=True, check=False)
        self.assertEqual(result.returncode, 0, result.stderr)
...
const primarySurface=new El(),secondarySurface=new El(),chatHost=new El(),chatHostB=new El(),questionB=new El(),chat={{'[data-evidence-ai-question]':ids['#fusion-evidence-ai-question'],'[data-evidence-ai-send]':ids['#fusion-evidence-ai'],'[data-evidence-ai-reason]':ids['#fusion-evidence-ai-reason'],'[data-evidence-ai-context]':ids['#fusion-evidence-ai-context'],'[data-evidence-ai-output]':ids['#fusion-evidence-ai-output'],'[data-evidence-ai-inline-progress]':new El()}};chatHost.querySelector=selector=>chat[selector]||null;chatHostB.querySelector=selector=>selector==='[data-evidence-ai-question]'?questionB:chat[selector]||null;secondarySurface.querySelectorAll=selector=>selector==='[data-evidence-chat]'?[chatHost,chatHostB]:[];ids['#fusion-primary-editor-surface']=primarySurface;ids['#fusion-secondary-editor-surface']=secondarySurface;
globalThis.document={{readyState:'loading',documentElement:{{dataset:{{}},style:{{setProperty(){{}}}}}},body:{{dataset:{{view:'search'}}}},querySelector:s=>ids[s]||null,querySelectorAll:s=>s==='[data-evidence-chat]'?[chatHost,chatHostB]:[],addEventListener(){{}}}};globalThis.localStorage={{getItem:()=>null,setItem(){{}}}};globalThis.innerWidth=1280;globalThis.confirm=()=>true;
globalThis.AutoResearchAIExperience={{AIExperienceController:class{{begin(){{return true}}update(){{return true}}definition(){{return null}}renderConversation(host,messages){{host.textContent=messages.map(message=>message.content).join('\\n');host.dataset.renderedRoles=messages.map(message=>message.role).join(',');return true}}}}}};
...
const tabA=api.documentTabs.open({{tabId:'evidence:a',kind:'evidence',ownerView:'search',title:'证据 A',identity:{{sourceScope:'official',sourceId:'official-main',entityType:'item',entityUid:'item:a'}},payload:{{row:A,evidenceIdentity:'official:official-main:item:a',status:'ready'}}}},{{groupId:'secondary',pin:true}});chatHost.dataset.evidenceTabId=tabA.tabId;api.renderEditorSurfaces();ids['#fusion-evidence-ai-question'].value='问题 A';const priorA=[{{role:'user',content:'先前问题 A'}},{{role:'assistant',content:'先前回答 A'}}];api.state.evidenceChat.messages=priorA.map(message=>({{...message}}));const paid=api.submitSelectedEvidenceAI();for(let attempt=0;attempt<40&&!releaseConsent;attempt+=1)await new Promise(resolve=>setImmediate(resolve));assert(releaseConsent,'authorization must reach the bounded consent wait');assert.equal(finishPaidJob,null,'execute must wait for consent');assert.deepEqual(outgoing[0],{{url:'/api/desktop/ai/actions/selected_evidence_chat/prepare',body:{{source_scope:'official',source_id:'official-main',entity_type:'item',entity_uid:'item:a',question:'问题 A',history:priorA}}}});assert.deepEqual(outgoing[1],{{url:'/api/desktop/ai/consents',body:{{action_id:'action_A_abcdefghijklmnopqrstuvwx'}}}});assert.equal(api.state.evidenceChat.pending.has(keyA),true);assert.equal(api.state.evidenceChat.busy,true);
const keyB='official:official-main:item:item:b',tabB=api.documentTabs.open({{tabId:'evidence:b',kind:'evidence',ownerView:'search',title:'证据 B',identity:{{sourceScope:'official',sourceId:'official-main',entityType:'item',entityUid:'item:b'}},payload:{{row:B,evidenceIdentity:'official:official-main:item:b',status:'ready'}}}},{{groupId:'secondary',pin:true}});chatHostB.dataset.evidenceTabId=tabB.tabId;api.renderEditorSurfaces();assert.equal(api.state.evidenceChat.identity,keyB);assert.equal(api.state.evidenceChat.busy,false,'background A must not mark current B busy');assert(ids['#fusion-evidence-ai-reason'].textContent.includes('另一条证据正在回答'));assert.equal(api.state.evidenceChat.messages.length,0,'B must not display A messages');questionB.value='问题 A';const beforeSecond=prepareCalls;assert.equal(await api.submitSelectedEvidenceAI(),null,'parallel paid request must be rejected');assert.equal(prepareCalls,beforeSecond,'parallel rejection must happen before prepare');
releaseConsent();for(let attempt=0;attempt<40&&!finishPaidJob;attempt+=1)await new Promise(resolve=>setImmediate(resolve));assert(finishPaidJob,'execution must resume within the bounded wait');assert.equal(questionB.value,'问题 A','authorizing A must preserve identical draft in B');assert.equal(ids['#fusion-evidence-ai-question'].value,'','only the original A input may clear');assert.deepEqual(outgoing[2],{{url:'/api/desktop/ai/actions/selected_evidence_chat/execute-jobs',body:{{action_id:'action_A_abcdefghijklmnopqrstuvwx',consent_nonce:'nonce_A_abcdefghijklmnopqrstuvwx'}}}});assert.equal(outgoing.length,3,'switching tabs must not prepare or execute B');finishPaidJob();await paid;assert.equal(questionB.value,'问题 A','A completion must not clear B either');assert.equal(api.state.evidenceChat.busy,false);assert.equal(api.state.evidenceChat.pending.size,0);assert.equal(api.state.evidenceChat.messages.length,0,'completed A must not appear in current B');const threadA=api.state.evidenceChat.threads.get(keyA);assert.deepEqual(threadA.map(message=>message.content),['先前问题 A','先前回答 A','问题 A','后台回答 A']);assert.deepEqual(threadA[3].annotations.pages,[7]);assert.deepEqual(threadA[3].annotations.notes,['第 7 页给出该数值。']);assert.deepEqual(threadA[3].annotations.limitations,['只解释当前证据。']);await api.state.evidenceChat.historySave;chatHost.dataset.evidenceTabId=tabA.tabId;assert(api.activateDocumentTab(tabA.tabId),'the original evidence tab must remain activatable');assert.equal(api.state.evidenceChat.identity,keyA);assert.deepEqual(api.state.evidenceChat.messages.map(message=>message.content),['先前问题 A','先前回答 A','问题 A','后台回答 A']);assert.equal(ids['#fusion-evidence-ai-output'].textContent,'先前问题 A\\n先前回答 A\\n问题 A\\n后台回答 A','reactivating A must repaint the real chat output for A');assert.equal(ids['#fusion-evidence-ai-output'].dataset.renderedRoles,'user,assistant,user,assistant');api.state.evidenceChat.messages=[{{role:'user',content:'仅清除 B'}}];api.state.evidenceChat.threads.set(keyB,[{{role:'user',content:'仅清除 B'}}]);chatHostB.dataset.evidenceTabId=tabB.tabId;assert(api.activateDocumentTab(tabB.tabId));api.state.evidenceChat.historyBackend='desktop-secure';api.state.evidenceChat.historyThreads.set(keyB,{{messages:[]}});globalThis.fetch=async(url,options={{}})=>{{assert.equal(String(url),'/api/desktop/evidence-chat-history');assert.equal(JSON.parse(options.body).operation,'delete');return response({{schema_version:'evidence-chat-history-v1',revision:1,storage:'macos-preview-local-key-aes-256-gcm',threads:[]}});}};assert.equal(await api.clearSelectedEvidenceThread(),true);assert.equal(api.state.evidenceChat.messages.length,0);assert.equal(api.state.evidenceChat.threads.has(keyB),false);assert.equal(api.state.evidenceChat.threads.has(keyA),true,'clearing B must preserve A thread');
}})().catch(error=>{{console.error(error);process.exitCode=1;}});
"""
        result = subprocess.run(["node", "-e", program], capture_output=True, text=True, check=False, timeout=8)
        self.assertEqual(result.returncode, 0, result.stderr)

    def test_initialization_restores_encrypted_evidence_history_to_matching_tab(self) -> None:
        program = f"""
(async()=>{{
const fs=require('fs'),assert=require('assert');
class Classes{{constructor(){{this.s=new Set()}}toggle(k,v){{v?this.s.add(k):this.s.delete(k)}}add(k){{this.s.add(k)}}remove(k){{this.s.delete(k)}}contains(k){{return this.s.has(k)}}}}
class El{{constructor(tag='div'){{this.tagName=String(tag).toUpperCase();this.hidden=false;this.disabled=false;this.textContent='';this.innerHTML='';this.value='';this.dataset={{}};this.attrs={{}};this.classList=new Classes();this.listeners={{}};this.children=[];this.isConnected=true;this.clientWidth=1280;this.scrollTop=0;this.style={{setProperty(){{}}}}}}setAttribute(k,v){{this.attrs[k]=String(v)}}removeAttribute(k){{delete this.attrs[k]}}addEventListener(k,fn){{(this.listeners[k]??=[]).push(fn)}}focus(){{globalThis.focused=this}}append(...nodes){{for(const node of nodes){{node.parentElement=this;this.children.push(node)}}}}replaceChildren(...nodes){{this.children=[];this.append(...nodes)}}remove(){{this.removed=true}}querySelector(){{return null}}querySelectorAll(){{return []}}closest(){{return null}}contains(){{return false}}matches(){{return false}}getBoundingClientRect(){{return{{width:this.clientWidth}}}}}}
const memory=new Map();globalThis.localStorage={{getItem:key=>memory.get(key)||null,setItem:(key,value)=>memory.set(key,String(value))}};globalThis.innerWidth=1280;globalThis.addEventListener=()=>{{}};globalThis.confirm=()=>true;
eval(fs.readFileSync({str(WEB / 'document_tab_store.js')!r},'utf8'));const Store=globalThis.AutoResearchDocumentTabs.DocumentTabStore,tabId='evidence:restored-a',keyA='official:official-main:item:item:a';const seed=new Store();seed.open({{tabId,kind:'evidence',ownerView:'search',title:'恢复的证据 A',identity:{{sourceScope:'official',sourceId:'official-main',entityType:'item',entityUid:'item:a'}}}},{{groupId:'secondary',pin:true}});
const ids=new Map(),node=selector=>{{if(!ids.has(selector))ids.set(selector,new El());return ids.get(selector)}};const output=node('#history-output'),question=node('#history-question'),send=node('#history-send'),reason=node('#history-reason'),context=node('#history-context'),status=node('#history-status'),progress=node('#history-progress'),body=node('#history-body'),expand=node('#history-expand'),clear=node('#history-clear'),chatHost=new El();chatHost.dataset.evidenceTabId=tabId;chatHost.querySelector=selector=>({{'[data-evidence-ai-output]':output,'[data-evidence-ai-question]':question,'[data-evidence-ai-send]':send,'[data-evidence-ai-reason]':reason,'[data-evidence-ai-context]':context,'[data-evidence-history-status]':status,'[data-evidence-ai-inline-progress]':progress,'[data-evidence-chat-body]':body,'[data-evidence-ai-expand]':expand,'[data-evidence-ai-clear]':clear}}[selector]||null);chatHost.querySelectorAll=()=>[];
const secondary=node('#fusion-secondary-editor-surface');secondary.querySelectorAll=selector=>selector==='[data-evidence-chat]'?[chatHost]:[];const root=node('documentElement'),pageBody=node('body');root.dataset={{}};pageBody.dataset={{view:'paper'}};let domReady=null;
globalThis.document={{readyState:'loading',activeElement:null,documentElement:root,body:pageBody,createElement:tag=>new El(tag),querySelector:selector=>node(selector),querySelectorAll:selector=>selector==='[data-evidence-chat]'?[chatHost]:[],addEventListener:(kind,handler)=>{{if(kind==='DOMContentLoaded')domReady=handler}}}};
globalThis.AutoResearchAIExperience={{AIExperienceController:class{{begin(){{return true}}update(){{return true}}definition(){{return null}}renderConversation(host,messages){{host.textContent=messages.map(message=>message.content).join('\\n');host.dataset.renderedRoles=messages.map(message=>message.role).join(',');return true}}}}}};
const calls=[],response=payload=>({{ok:true,headers:{{get:()=>null}},json:async()=>payload}});globalThis.fetch=async(url)=>{{url=String(url);calls.push(url);const path=url.split('?')[0];if(path==='/api/desktop/evidence-chat-history')return response({{schema_version:'evidence-chat-history-v1',revision:4,storage:'macos-preview-local-key-aes-256-gcm',threads:[{{schema_version:'evidence-chat-thread-v1',thread_uid:'ech_server_derived',source_scope:'official',source_id:'official-main',entity_type:'item',entity_uid:'item:a',title:'恢复的证据 A',created_at:'2026-08-30T00:00:00Z',updated_at:'2026-08-31T00:00:00Z',messages:[{{role:'user',content:'重启前的问题'}},{{role:'assistant',content:'重启前的回答',annotations:{{pages:[7],notes:['第 7 页'],limitations:['仅当前证据']}}}}]}}]}});if(path==='/api/desktop/federated-evidence')return response({{entity_type:'item',source_scope:'official',source_id:'official-main',entity_uid:'item:a',display_title:'恢复的证据 A',value_text:'4.1',unit:'GPa',meaning:'硬度',source_page:7,source_excerpt:'原文证据',article_title:'论文 A'}});if(path==='/api/search-papers')return response([]);if(path==='/api/desktop/librarian-history')return response({{storage:'macos-preview-local-key-aes-256-gcm',sessions:[]}});if(path==='/api/desktop/research-memories')return response({{schema_version:'research-memory-list-v1',storage:'macos-preview-local-key-aes-256-gcm',revision:0,items:[]}});if(path==='/api/desktop/review-queue')return response({{schema_version:'review-queue-v1',source_scope:'workspace',source_id:'workspace',paper_uid:null,total:0,items:[]}});if(path==='/api/desktop/ai/actions/literature_extraction/task-directory')return response({{schema_version:'literature-extraction-task-directory-v1',tasks:[],startup_recovery:{{recovered:0,already_completed:0,skipped_or_blocked:0}},issues:[]}});if(path==='/api/desktop/ai/actions/literature_extraction/jobs')return response({{schema_version:'ai-execution-job-list-v1',scope:'literature_extraction',jobs:[]}});return response({{}})}};
eval(fs.readFileSync({str(WEB / 'fusion_review.js')!r},'utf8'));const api=globalThis.AutoResearchFusion;assert(domReady,'Fusion must register its page initialization handler');domReady();for(let attempt=0;attempt<120&&(api.state.evidenceChat.identity!==keyA||api.state.evidenceChat.messages.length!==2);attempt+=1)await new Promise(resolve=>setImmediate(resolve));
assert.equal(calls.filter(url=>url==='/api/desktop/evidence-chat-history').length,1,'page initialization must load encrypted evidence history once');assert.equal(api.documentTabs.activeTab('secondary').tabId,tabId);assert.equal(api.state.evidenceChat.identity,keyA);assert.deepEqual(api.state.evidenceChat.messages.map(message=>message.content),['重启前的问题','重启前的回答']);assert.equal(output.textContent,'重启前的问题\\n重启前的回答','the restored entity thread must render in its matching evidence tab');assert.equal(output.dataset.renderedRoles,'user,assistant');assert.equal(api.state.evidenceChat.historyBackend,'desktop-secure');assert.equal(api.state.evidenceChat.threads.has(keyA),true);
}})().catch(error=>{{console.error(error);process.exitCode=1;}});
"""
        result = subprocess.run(
            ["node", "-e", program], capture_output=True, text=True, check=False, timeout=20
        )
        self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_table_structure_review_is_scoped_and_path_free(self) -> None:
        for marker in (
            ...
            'data-table-structure-candidate="auto"',
            'data-table-structure-candidate="manual"',
            "在主栏审核",
            "结构 CSV",
            "结构 XLSX",
            "官方资料库已核验 · 同源只读",
        ...
        for marker in (
            ...
            ".fusion-table-structure-create textarea",
            "var(--f-accent)",
        ):
            self.assertIn(marker, self.css)
        review_body = self.runtime.split(
            'const body={entity_uid:identity.entityUid,expected_version:structure.version,operation,note:""}',
            1,
    ...
    def test_table_structure_runtime_keeps_sources_tabs_and_late_results_isolated(self) -> None:
        program = f"""
const fs=require('fs'),assert=require('assert');
...
const dto={{schema_version:'review-queue-v1',source_scope:'workspace',source_id:'workspace',paper_uid:null,total:1,items:[{{review_token:token,expires_at:1999999999,source_scope:'workspace',source_id:'workspace',paper_uid:paperUid,paper:{{title:'W-Ta 论文',doi:'10.1/example'}},entity_type:'item',candidate:{{value_text:'4.1',unit:'GPa',source_page:7,source_excerpt:'原文短摘录',paper_id:99,image_path:'/private/a.png',reviewer:'secret'}},alternate:{{value_text:'4.0',unit:'GPa',source_page:7,source_excerpt:'备选短摘录'}},conflict_reason:'数值不一致',scores:{{agreement:0,factuality:70,completeness:100,evidence:100,overall:69.5}},allowed_operations:{{approve:{{available:true}},reject:{{available:true}},correct:{{available:true}},merge:{{available:false}},split:{{available:false}}}}}}]}};
const projected=api.publicReviewQueue(dto);assert(projected);assert.equal(projected.total,1);const item=projected.items[0];assert.equal(item.paperUid,paperUid);assert.equal(item.candidate.value_text,'4.1');assert.equal(item.candidate.source_page,7);assert.equal(item.scores.agreement,0);assert.equal(item.scores.factuality,.7);assert.equal(item.scores.completeness,1);assert.equal(item.scores.overall,.695);assert.equal(item.candidate.paper_id,undefined);assert.equal(item.candidate.image_path,undefined);assert.equal(item.candidate.reviewer,undefined);
const html=api.reviewCandidateHTML(item);for(const text of ['主候选','备选','冲突原因','五项评分','70%','原文页','原文短摘录','批准','不采用','确认纠正'])assert(html.includes(text),text);assert(!html.includes(token));assert(!html.includes('/private/a.png'));assert(!html.includes('secret'));
assert.equal(api.publicReviewQueue({{...dto,total:2}}),null);assert.equal(api.publicReviewQueue({{...dto,items:[{{...dto.items[0],entity_type:'unknown'}}]}}),null);assert.equal(api.publicReviewQueue({{...dto,items:[{{...dto.items[0],paper_uid:'paper_bad'}}]}}),null);assert.equal(api.publicReviewQueue({{...dto,items:[{{...dto.items[0],scores:{{...dto.items[0].scores,overall:100.1}}}}]}}),null);assert.equal(api.publicReviewQueue({{...dto,items:[{{...dto.items[0],scores:{{...dto.items[0].scores,overall:-.1}}}}]}}),null);assert.equal(api.publicReviewQueue({{...dto,items:[{{...dto.items[0],scores:{{...dto.items[0].scores,overall:'69.5'}}}}]}}),null);
"""
        result = subprocess.run(["node", "-e", program], capture_output=True, text=True, check=False)
        self.assertEqual(result.returncode, 0, result.stderr)

    def test_review_queue_failure_retries_and_catalog_refresh_does_not_steal_view(self) -> None:
        program = f"""
globalThis.document={{readyState:'loading',querySelector:()=>null,querySelectorAll:()=>[],addEventListener:()=>{{}}}};
globalThis.localStorage={{getItem:()=>null,setItem:()=>{{}}}};globalThis.addEventListener=()=>{{}};
const assert=require('assert'),fs=require('fs'),token='rq_'+('A'.repeat(40)),paperUid='paper_0123456789abcdef0123456789abcdef';
const queue={{schema_version:'review-queue-v1',source_scope:'workspace',source_id:'workspace',paper_uid:null,total:1,items:[{{review_token:token,expires_at:1999999999,source_scope:'workspace',source_id:'workspace',paper_uid:paperUid,paper:{{title:'已发布论文',doi:'10.1/new'}},entity_type:'finding',candidate:{{finding_text:'真实结论',source_page:2,source_excerpt:'原文'}},alternate:null,conflict_reason:'需要人工核对',scores:{{agreement:0,factuality:70,completeness:100,evidence:100,overall:69.5}},allowed_operations:{{approve:{{available:true}},reject:{{available:true}},correct:{{available:false}}}}}}]}};
...
;(async()=>{{
 ...
 assert.equal(await api.loadReviewQueue(true),false);assert.equal(api.state.reviewQueue.loaded,false);assert.equal(api.state.reviewQueue.total,null);assert(api.state.reviewQueue.error.includes('review_queue_unavailable'));assert(!api.state.reviewQueue.error.includes('private transport detail'));
 assert.equal(await api.loadReviewQueue(true),true);assert.equal(api.state.reviewQueue.loaded,true);assert.equal(api.state.reviewQueue.total,1);assert.equal(api.state.reviewQueue.error,'');
 api.state.reviewQueue.loaded=true;api.state.reviewQueue.total=0;api.state.reviewQueue.items=[];api.invalidateReviewQueue({{refresh:true}});await new Promise(resolve=>setImmediate(resolve));assert.equal(api.state.reviewQueue.total,1);assert.equal(calls.filter(url=>url==='/api/desktop/review-queue').length,3,'one invalidation issues exactly one refresh');
 api.state.view='search';api.state.paper={{id:7,title:'已发布论文',status:'未扫描'}};api.state.papers=[api.state.paper];api.state.evidence=[1,2,3,4];assert.equal(await api.refreshLiteratureCatalog(),true);assert.equal(api.state.view,'search','catalog refresh must not steal navigation');assert.equal(api.state.paper.status,'已扫描');assert.equal(api.state.paper.count,4);
 let releaseCatalog=null;globalThis.fetch=url=>new Promise(resolve=>{{assert.equal(String(url),'/api/search-papers');releaseCatalog=()=>resolve(success([{{id:7,title:'已发布论文',doi:'10.1/new',six_row_count:4,six_workflow_label:'已扫描'}},{{id:8,title:'用户已切换论文',doi:'10.1/current',six_row_count:9,six_workflow_label:'已扫描'}}]));}});api.state.paper={{id:7,title:'已发布论文',status:'未扫描'}};const lateCatalog=api.refreshLiteratureCatalog();api.state.paper={{id:8,title:'用户已切换论文',status:'当前选择B'}};releaseCatalog();assert.equal(await lateCatalog,true);assert.equal(api.state.paper.id,8);assert.equal(api.state.paper.status,'当前选择B','late catalog response must not replace the paper selected after the request started');assert(api.state.papers.some(paper=>paper.id===7&&paper.status==='已扫描'),'the full catalog may still refresh safely');
}})().catch(error=>{{console.error(error);process.exitCode=1;}});
"""
        result = subprocess.run(
            ["node", "-e", program], capture_output=True, text=True, timeout=8, check=False
    ...
    def test_review_queue_safety_contract_is_bounded(self) -> None:
        ...
        for marker in (
            ...
            "state.reviewQueue.total=null", "state.reviewQueue.loaded=false",
            'invalidateReviewQueue({refresh:true});void refreshLiteratureCatalog()',
            "待审核数量未知", "不会把未知状态显示为 0",
