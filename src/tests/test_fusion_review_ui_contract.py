...
finishPaidJob();await paid;assert.equal(api.state.evidenceChat.busy,false);assert.equal(api.state.evidenceChat.pending.size,0);assert.equal(api.state.evidenceChat.messages.length,0,'completed A must not appear in current B');const threadA=api.state.evidenceChat.threads.get(keyA);assert.deepEqual(threadA.map(message=>message.content),['问题 A','后台回答 A']);assert.deepEqual(threadA[1].annotations.pages,[7]);assert.deepEqual(threadA[1].annotations.notes,['第 7 页给出该数值。']);assert.deepEqual(threadA[1].annotations.limitations,['只解释当前证据。']);await api.state.evidenceChat.historySave;chatHost.dataset.evidenceTabId=tabA.tabId;assert(api.activateDocumentTab(tabA.tabId),'the original evidence tab must remain activatable');assert.equal(api.state.evidenceChat.identity,keyA);assert.deepEqual(api.state.evidenceChat.messages.map(message=>message.content),['问题 A','后台回答 A']);assert.equal(ids['#fusion-evidence-ai-output'].textContent,'问题 A\\n后台回答 A','reactivating A must repaint the real chat output for A');assert.equal(ids['#fusion-evidence-ai-output'].dataset.renderedRoles,'user,assistant');api.state.evidenceChat.messages=[{{role:'user',content:'仅清除 B'}}];api.state.evidenceChat.threads.set(keyB,[{{role:'user',content:'仅清除 B'}}]);chatHost.dataset.evidenceTabId=tabB.tabId;assert(api.activateDocumentTab(tabB.tabId));api.state.evidenceChat.historyBackend='desktop-secure';api.state.evidenceChat.historyThreads.set(keyB,{{messages:[]}});globalThis.fetch=async(url,options={{}})=>{{assert.equal(String(url),'/api/desktop/evidence-chat-history');assert.equal(JSON.parse(options.body).operation,'delete');return response({{schema_version:'evidence-chat-history-v1',revision:1,storage:'macos-preview-local-key-aes-256-gcm',threads:[]}});}};assert.equal(await api.clearSelectedEvidenceThread(),true);assert.equal(api.state.evidenceChat.messages.length,0);assert.equal(api.state.evidenceChat.threads.has(keyB),false);assert.equal(api.state.evidenceChat.threads.has(keyA),true,'clearing B must preserve A thread');
}})().catch(error=>{{console.error(error);process.exitCode=1;}});
"""
        result = subprocess.run(["node", "-e", program], capture_output=True, text=True, check=False)
        self.assertEqual(result.returncode, 0, result.stderr)
