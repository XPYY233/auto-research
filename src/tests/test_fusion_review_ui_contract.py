...
let pendingResolvers=[],reviewCount=0,failTable=false;const calls=[];globalThis.fetch=async(url,options={{}})=>{{url=String(url);calls.push([url,options.method||'GET',options.body]);const headers={{get:()=> 'csrf-next'}};
...
eval(fs.readFileSync({str(WEB / 'ai_consent.js')!r},'utf8'));eval(fs.readFileSync({str(WEB / 'document_tab_store.js')!r},'utf8'));eval(fs.readFileSync({str(WEB / 'pane_layout_controller.js')!r},'utf8'));eval(fs.readFileSync({str(WEB / 'workspace_layout_controller.js')!r},'utf8'));eval(fs.readFileSync({str(WEB / 'fusion_pdf_controller.js')!r},'utf8'));eval(fs.readFileSync({str(WEB / 'fusion_personal_import.js')!r},'utf8'));eval(fs.readFileSync({str(WEB / 'fusion_review.js')!r},'utf8'));const api=globalThis.AutoResearchFusion;
 ...
 failTable=false;await api.confirmPersonalImport();assert.equal(ids['#fusion-reviewed-state'].textContent,'✓ 已核验并保存');assert(ids['#fusion-personal-status'].textContent.includes('数据已保存，但搜索索引待恢复'));assert(!ids['#fusion-personal-status'].textContent.includes('导入未完成'));globalThis.innerWidth=1024;assert(await api.openImportedPersonalTable({{kind:'open_personal_table',source_scope:'private',source_id:'personal-lab',entity_type:'table',entity_uid:'private:table:one',label:'打开刚导入的表格'}},api.state.personalAction));assert.equal(api.workspaceProjection().secondary,'single');assert.equal(api.documentTabs.snapshot().activeGroupId,'secondary');const beforeReturnTabs=api.documentTabs.snapshot().tabs.length;assert.equal(api.returnFromSecondaryPersonalTable(),true);assert.equal(api.documentTabs.snapshot().activeGroupId,'primary');assert.equal(api.documentTabs.snapshot().tabs.length,beforeReturnTabs,'return must preserve imported table tab');assert(api.documentTabs.snapshot().tabs.some(tab=>tab.identity?.entityUid==='private:table:one'));globalThis.innerWidth=720;assert(await api.openImportedPersonalTable({{kind:'open_personal_table',source_scope:'private',source_id:'personal-lab',entity_type:'table',entity_uid:'private:table:one',label:'打开刚导入的表格'}},api.state.personalAction));personalTab=api.documentTabs.snapshot().tabs.find(tab=>tab.identity?.entityUid==='private:table:one');assert.equal(personalTab.groupId,'primary');assert.equal(personalTab.title,'刚导入的真实表格');assert.equal(api.state.evidenceDetailOpen,true);assert.equal(api.closeEvidenceDetail({{focus:false}}),true);assert.equal(api.state.view,'personal');
