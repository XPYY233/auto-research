...
assert(controller.begin('literature_extraction'));assert.equal(elapsed.textContent,'本次活动 0 秒（不含等待确认）');
...
assert.equal(label.textContent,'核对覆盖率与来源页');assert.equal(valueNode.textContent,'进行中');assert.equal(bar.style.width,'58%');assert.equal(track.attrs['aria-valuenow'],undefined);assert.equal(track.attrs['aria-valuetext'],'核对覆盖率与来源页');assert.equal(progress.attrs['aria-busy'],'true');assert.equal(detail.textContent,'真实来源页核验');
...
assert(controller.activity('literature_extraction',{{schema_version:'ai-activity-event-v1',job_id:'ai_job_abcdefghijklmnopqrstuvwxyz',sequence:2,code:'literature_third_review',stage:'third_review',label:'处理低置信与冲突结果',detail:''}}));assert.equal(valueNode.textContent,'进行中');assert.equal(label.textContent,'处理低置信与冲突结果');
controller.update('literature_extraction',{{stage:'completed',state:'success'}});assert.equal(valueNode.textContent,'已完成');assert.equal(progress.attrs['aria-busy'],'false');
controller.update('literature_extraction',{{stage:'coverage_verification',state:'error',detail:'来源页核验未完成'}});assert.equal(valueNode.textContent,'未完成');assert.equal(label.textContent,'核对覆盖率与来源页');assert.equal(detail.textContent,'来源页核验未完成');assert.equal(progress.attrs['aria-busy'],'false');
...
"""
    ...
    def test_elapsed_freezes_at_terminal_and_excludes_authorization_wait(self) -> None:
        program = r"""
const assert=require('assert'),fs=require('fs');
let now=0,visible=true,nextTimer=0;const timers=new Map();
Date.now=()=>now;
...
controller.begin(scope);assert.equal(timers.size,1);now=4000;controller.renderElapsed(scope);
assert.equal(elapsed.textContent,'本次活动 4 秒（不含等待确认）','zero epoch is a valid activity start');
controller.update(scope,{stage:'authorization',state:'waiting'});assert.equal(timers.size,0);assert.equal(value.textContent,'等待确认');
now=64000;controller.renderElapsed(scope);assert.equal(elapsed.textContent,'本次活动 4 秒（不含等待确认）');
controller.update(scope,{stage:'reasoning',state:'running'});controller.update(scope,{stage:'reasoning',state:'running'});assert.equal(timers.size,1,'resume must not duplicate intervals');
now=67000;controller.update(scope,{stage:'completed',state:'success'});const completed=elapsed.textContent;
...
controller.reset(scope);assert.equal(elapsed.textContent,'本次活动 0 秒（不含等待确认）');assert.equal(controller.elapsedMs.has(scope),false);
controller.begin(scope);now+=1000;controller.renderElapsed(scope);assert.equal(elapsed.textContent,'本次活动 1 秒（不含等待确认）','new task must not inherit old duration');
controller.stopTimer(scope);assert.equal(track.attrs['aria-valuenow'],undefined);assert(!value.textContent.includes('%'),'stage weights must not claim measured completion percentages');
""".replace("RUNTIME_PATH", repr(str(RUNTIME)))
        result = subprocess.run(
...
globalThis.document.querySelector=selector=>selector.endsWith('-progress')?progress:null;
for(const scope of ['personal_suggestion','literature_extraction','selected_evidence_chat']){{
  for(const [index,code] of ['execution_started','result_validating','execution_completed'].entries())assert(api.projectAIActivity(scope,{{...event(index+1,code),job_id:'ai_job_'+scope}}));
  const text=activity.children.map(node=>node.children[1].children.map(child=>child.textContent).join(' ')).join(' ');
  assert(!text.includes('回答与引用已就绪'),scope+' must not use librarian completion wording');
  if(scope==='personal_suggestion'){{assert(text.includes('校验列建议'));assert(text.includes('尚未确认导入'));assert(!text.includes('核验引用'));}}
  if(scope==='literature_extraction'){{assert(text.includes('核验提取结果'));assert(text.includes('发布与索引状态请查看提取收据'));}}
  if(scope==='selected_evidence_chat')assert(text.includes('当前证据回答已就绪'));
}}
"""
        result = subprocess.run(
            ["node", "-e", program], capture_output=True, text=True, check=False
