...
const now=Date.now(),historySessions=Array.from({{length:25}},(_,index)=>({{id:index===0?'old-session':`session-${{index}}`,title:index===0?'旧对话':`对话 ${{index}}`,created_at:new Date(now-index*1000).toISOString(),updated_at:new Date(now-index*1000).toISOString(),messages:[{{role:'user',content:index===0?'旧问题':`问题 ${{index}}`}},{{role:'assistant',content:index===0?'旧回答':`回答 ${{index}}`}}],results:[],articles:[]}}));historySessions.push({{...historySessions[0],updated_at:new Date(now-100000).toISOString()}},{{id:'expired',updated_at:new Date(now-31*24*60*60*1000).toISOString(),messages:[{{role:'user',content:'过期'}}]}},{{id:'future',updated_at:new Date(now+10*60*1000).toISOString(),messages:[{{role:'user',content:'未来'}}]}},{{id:'invalid',updated_at:'not-a-time',messages:[{{role:'user',content:'无效'}}]}});
let savedHistory=null,historyCleared=false,memoryRevision=0,memoryItems=[],historyGetCount=0,historyStorage='macos-preview-local-key-aes-256-gcm',releaseLateHistory=null;const headers={{get:()=>null}};
globalThis.fetch=async(url,options={{}})=>{{
 url=String(url);const method=options.method||'GET';
 if(url==='/api/desktop/librarian-history'&&method==='GET'){{historyGetCount+=1;const response={{ok:true,headers,json:async()=>({{storage:historyStorage,sessions:historySessions}})}};if(historyGetCount===2)return new Promise(resolve=>{{releaseLateHistory=()=>resolve(response);}});return response;}}
...
assert.equal(await api.loadLibrarianHistory(),20);assert.equal(new Set(api.state.librarianSessions.map(session=>session.id)).size,20);assert(!api.state.librarianSessions.some(session=>['expired','future','invalid'].includes(session.id)));assert(api.restoreLibrarianSession('old-session'));assert.equal(api.state.librarianMessages[0].content,'旧问题');
api.state.view='package';const lateLoad=api.loadLibrarianHistory();await Promise.resolve();assert.equal(api.state.view,'package');releaseLateHistory();await lateLoad;assert.equal(api.state.view,'package','late history response must not steal the active module');
historyStorage='plaintext';assert.equal(await api.loadLibrarianHistory(),0);assert.equal(api.state.librarianHistoryBackend,'unavailable');assert.equal(api.state.librarianSessions.length,0);assert.equal(api.state.view,'package','fail-closed history projection must not navigate');historyStorage='macos-preview-local-key-aes-256-gcm';assert.equal(await api.loadLibrarianHistory(),20);
