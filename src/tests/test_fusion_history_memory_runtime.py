...
const ids={{}};for(const id of ['fusion-librarian-history','fusion-librarian-history-privacy','fusion-librarian-stage','fusion-librarian-question','fusion-librarian-status','fusion-librarian-output','fusion-librarian-results-body','fusion-librarian-results-count','fusion-librarian-results','fusion-librarian-results-toggle','fusion-librarian-panel','fusion-research-memory-list','fusion-research-memory-status','fusion-research-memory-clear','fusion-librarian-remember','fusion-librarian-current-session'])ids['#'+id]=new El();
globalThis.document={{readyState:'loading',querySelector:s=>ids[s]||null,querySelectorAll:()=>[],addEventListener(){{}},createElement:()=>new El(),documentElement:{{dataset:{{}},style:{{setProperty(){{}}}}}},body:{{dataset:{{view:'search'}}}}}};globalThis.localStorage={{getItem:()=>null,setItem(){{}}}};globalThis.confirm=()=>true;
let savedHistory=null,memoryRevision=0,memoryItems=[];const headers={{get:()=>null}};
globalThis.fetch=async(url,options={{}})=>{{
 ...
 if(url==='/api/desktop/librarian-history'&&method==='POST'){{savedHistory=JSON.parse(options.body).sessions;return{{ok:true,headers,json:async()=>({{ok:true}})}};}}
