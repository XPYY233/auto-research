...
ids['#fusion-personal-status']=new El();
...
assert(table.head.innerHTML.includes('aria-rowindex="1"'));assert.deepEqual([...table.body.innerHTML.matchAll(/aria-rowindex="([0-9]+)"/g)].map(match=>Number(match[1])),Array.from({{length:50}},(_,index)=>index+2));assert.equal(table.attrs['aria-rowcount'],'52');assert.equal(table.attrs['aria-busy'],'false');assert(ids['#fusion-personal-status'].textContent.includes('已读取所选工作表第 1–50 行'));assert(!ids['#fusion-personal-status'].textContent.includes('正在读取'));
...
assert(table.body.innerHTML.includes('aria-rowindex="52"'));assert.equal(table.attrs['aria-rowcount'],'52');assert(ids['#fusion-personal-status'].textContent.includes('第 51–51 行 / 共 51 行'));
...
assert.equal(table.attrs['aria-rowcount'],'2','empty grid includes header and its explanatory row');assert(table.body.innerHTML.includes('aria-rowindex="2"'));assert.equal(table.attrs['aria-busy'],'false');assert(ids['#fusion-personal-status'].textContent.includes('当前工作表为空'));
...
assert(ids['#fusion-personal-status'].textContent.includes('共 1 行'),'late page must not overwrite completed status');
const errorPage=api.loadPersonalPreviewPage(1,1);assert.equal(table.attrs['aria-rowcount'],'2');assert.equal(table.attrs['aria-busy'],'true');assert(table.head.innerHTML.includes('aria-rowindex="1"'));assert(table.body.innerHTML.includes('aria-rowindex="2"'));assert(ids['#fusion-personal-status'].textContent.includes('正在读取'));
pending[2].resolve({{ok:false,headers,json:async()=>({{code:'personal_page_unavailable',message:'不可用'}})}});assert.equal(await errorPage,false);assert.equal(table.attrs['aria-busy'],'false');assert.equal(table.attrs['aria-rowcount'],'2');assert.equal(ids['#fusion-personal-page-status'].dataset.state,'error');assert(!ids['#fusion-personal-status'].textContent.includes('正在读取'));
