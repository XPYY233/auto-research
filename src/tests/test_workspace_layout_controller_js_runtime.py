...
class WorkspaceLayoutControllerRuntimeTest(unittest.TestCase):
    def test_projection_obeys_page_and_column_contract(self):
        program = textwrap.dedent(
            ...
            assert.equal(value.context,'docked');assert.equal(value.secondary,'hidden');assert.equal(value.inspector,'hidden');assert.equal(value.librarianResults,'hidden');assert.equal(value.dockedColumns,2);
            ...
            assert.equal(value.context,'docked');assert.equal(value.secondary,'hidden');assert.equal(value.inspector,'hidden');assert.equal(value.librarianResults,'hidden');assert.equal(value.dockedColumns,2);
            value=layout.project({{width:1280,view:'search',searchMode:'librarian',librarianResultsOpen:true,hasSecondary:true,hasInspectorSelection:true}});
            assert.equal(value.context,'drawer');assert.equal(value.secondary,'hidden');assert.equal(value.inspector,'hidden');assert.equal(value.librarianResults,'docked');assert.equal(value.dockedColumns,2);
