...
const ready=state=>({{state,reason_code:state==='ready'?'ai_business_ready':'ai_business_verification_required',next_action:state==='ready'?'none':'test_business_capability'}});let personalState='verification_required',custom={{schema_version:'custom-ai-provider-v1',provider_id:'custom',revision:0,configured:false}},confirmValue=true;globalThis.confirm=()=>confirmValue;const calls=[];
...
globalThis.fetch=async(url,options={{}})=>{{url=String(url);calls.push([url,options.method||'GET',options.body]);
 ...
 if(url==='/api/desktop/ai/providers/deepseek/test'){{personalState='ready';return response({{schema_version:'ai-capability-test-result-v1',status:'verified'}});}}
