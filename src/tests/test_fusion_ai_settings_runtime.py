...
globalThis.fetch=async(url,options={{}})=>{{url=String(url);calls.push([url,options.method||'GET',options.body]);
 ...
 if(url==='/api/desktop/ai/actions/personal_suggestion/execute'){{if(personalFailure)return failure({{schema_version:'desktop-ai-http-error-v1',code:'ai_execution_failed',cause_code:'harness_budget_exhausted',message:'transport canary secret path',stage:'personal_execute',next_action:'retry_same_request',retryable:true}});return response({{schema_version:'personal-import-suggestion-v1',import_id:'personal_import_abcdefghijklmnop',columns:[]}});}}
