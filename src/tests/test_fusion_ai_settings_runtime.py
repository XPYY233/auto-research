...
const ready=state=>({{state,reason_code:state==='ready'?'ai_business_ready':'ai_business_verification_required',next_action:state==='ready'?'none':'test_business_capability'}});let connectionState='ready',personalState='verification_required',custom={{schema_version:'custom-ai-provider-v1',provider_id:'custom',revision:0,configured:false}},confirmValue=true,testFailure=false;globalThis.confirm=()=>confirmValue;const calls=[];
