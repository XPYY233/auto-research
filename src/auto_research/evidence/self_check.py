...
def _requirement_summary(checks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        {
            "id": "fusion_functional_literature",
            "ok": ok("web_ui_contract", "fuzzy_search", "source_highlight"),
            "requirement": "Fusion工作台把 PDF 导入、自动提取、原文返回和可定位证据集中在同一文献工作区。",
            "evidence": details("web_ui_contract"),
        },
        {
            "id": "fusion_personal_experiment_import",
            "ok": ok("web_ui_contract"),
            "requirement": "Fusion实验页以显眼入口选择真实表格，安全预览后由用户一次确认进入私人库；合成表仅作未选择文件时的示例。",
            "evidence": details("web_ui_contract"),
        },
        {
        ...
        {
            "id": "fusion_controlled_ai_actions",
            "ok": ok("web_ui_contract"),
            "requirement": "图书管理员、文献提取和实验 AI 建议只走服务端准备、版本化披露、一次性授权和受控执行；取消不伪造成功。",
            "evidence": details("web_ui_contract"),
        },
    ]
