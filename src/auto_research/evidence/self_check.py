...
def _web_ui_contract() -> dict[str, Any]:
    ...
    expectations = [
        ...
        ("search_engine", "id=\"search-form\"" in html and "/api/six-search" in js and "选择检索对象" in html),
        ("three_mode_visual_search", all(mode in html for mode in ("条目搜索", "表格搜索", "图片搜索")) and "/api/visual-search" in js and "setSearchMode" in js),
        ...
        ("table_result_collapsing", "table-result-group" in js and "同一原表集中显示" in js and "展开其余" in js),
