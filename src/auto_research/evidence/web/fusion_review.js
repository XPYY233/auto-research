(() => {
  ...
  const ROUTES = Object.freeze({papers:"/api/search-papers",evidence:"/api/search-v2",federatedSearch:"/api/desktop/federated-search",settings:"/api/desktop/settings",preferences:"/api/desktop/settings/preferences",aiProviders:"/api/desktop/ai/providers",aiSettings:"/api/desktop/ai/settings",upload:"/api/uploads/pdf",consent:"/api/desktop/ai/consents",personalPreview:"/api/desktop/personal-imports/preview"});
  ...
  function isAllowedRoute(url,method) {
    ...
    if(method==="GET")return [ROUTES.papers,ROUTES.evidence,ROUTES.federatedSearch,ROUTES.settings,ROUTES.aiProviders,ROUTES.aiSettings].includes(path)||/^\/api\/desktop\/ai\/credentials\/(deepseek|openai)$/.test(path);
  ...
  async function readOnlyJSON(url, signal) {
    if (![ROUTES.papers,ROUTES.evidence,ROUTES.federatedSearch,ROUTES.settings,ROUTES.aiProviders,ROUTES.aiSettings].some(route => url === route || url.startsWith(`${route}?`))) throw safeError("fusion_read_route_invalid","只读请求无效。");
  ...
  async function selectPaper(id) {
    const paper=state.papers.find(candidate=>candidate.id===id);if(!paper)return false;state.literatureAction+=1;closeCurrentPDF({focus:false});state.paper=paper;renderPaperCatalog();
  ...
  async function runPreciseSearch() {const query=String(q("#fusion-search-query")?.value||"").trim(),source=state.searchSource,generation=++state.searchRequest;setSearchMode("precise");q("#fusion-search-results").innerHTML=literatureState("loading","正在检索四类真实证据…");try{const federated=source!=="workspace",scope=source==="official"?"&source_scope=official":source==="private"?"&source_scope=private":"",url=federated?`${ROUTES.federatedSearch}?q=${encodeURIComponent(query)}&page=1&page_size=100${scope}`:`${ROUTES.evidence}?q=${encodeURIComponent(query)}&limit=100`,page=await readOnlyJSON(url);if(generation!==state.searchRequest||state.view!=="search"||state.searchMode!=="precise"||source!==state.searchSource)return;const raw=federated?(Array.isArray(page?.results)?page.results.map(hit=>hit?.document):[]):(Array.isArray(page?.rows)?page.rows:[]);state.searchResults=raw.map(publicEvidence).filter(Boolean);state.evidenceIndex=0;state.selectedEvidence=state.searchResults[0]||null;renderSearchEvidence();updateInspector();const sourceLabel={workspace:"本机文献",official:"官方资料库",private:"我的实验",all:"全部资料源"}[source];setOperation(`${sourceLabel}检索完成 · ${state.searchResults.length} 条`,"success");}catch(_error){if(generation===state.searchRequest)q("#fusion-search-results").innerHTML=literatureState("error","当前资料源检索暂时不可用");}}
  ...
  function switchView(name,{focus=true}={}) {
    ...
    const previous=state.view;if(previous!==name){if(previous==="paper"){state.literatureAction+=1;closeCurrentPDF({focus:false});}if(previous==="search"){state.searchRequest+=1;cancelLibrarian("已离开图书管理员，未继续后续调用。");}if(previous==="personal")state.personalAction+=1;} state.view=name; document.body.dataset.view=name;
  ...
  globalThis.AutoResearchFusion=Object.freeze({initialize,switchView,renderSheet,selectCell,selectPaper,selectEvidence,loadLiterature,loadSettings,hydrateSettings,chooseAppearance,openDrawer,closeDrawers,openCommandPalette,closeCommandPalette,applyAppearance,updateColumnDefinition,selectSettingsSection,runPreciseSearch,setSearchMode,setSearchSource,uploadPDF,runLiteratureExtraction,openCurrentPDF,closeCurrentPDF,choosePersonalFile,requestPersonalSuggestion,confirmPersonalImport,preparedAuthorization,loadAISettingsUI,saveAIModels,saveAIKey,deleteAIKey,testAIConnection,previewCellValue,state,syntheticSheets});
