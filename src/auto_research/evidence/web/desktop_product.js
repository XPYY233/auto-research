(() => {
  ...
  function openPackageCenter() {
    if (!product.available) return;
    void packageCenter?.open();
  }

  function packageCenterLiteratureSnapshot() {
    ...
    if (String(filters.query || "").trim()) filter.query = String(filters.query).trim();
    if (String(filters.author || "").trim()) filter.author = String(filters.author).trim();
    if (filters.topic && filters.topic !== "all") filter.topic = filters.topic;
    if (filters.status && filters.status !== "all") filter.status = filters.status;
    if (filters.scope && filters.scope !== "all") filter.scope = filters.scope;
    return Object.freeze({
      selectedIds: Object.freeze([...(state.selectedPaperIds || [])].map(value => String(value))),
      filter: Object.freeze(filter),
      paperCount: Number(state.papers?.length || 0),
    });
  }

  async function runPackageCenterRollback(packageId, packageVersion) {
    const response = await api("/api/desktop/evidence-packages/rollback", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ package_id: packageId, target_version: packageVersion }),
    });
    await waitForJob(response.job_id);
    await loadPackageStatus();
  }

  async function selectPackageForCenter() {
    if (typeof window.pywebview?.api?.select_evidence_package !== "function") {
      throw new Error("请在 Auto Research 桌面 App 中选择资料包。");
    }
    return window.pywebview.api.select_evidence_package();
  }

  async function selectPackageExportDestination(filename) {
    if (typeof window.pywebview?.api?.select_package_export_destination !== "function") {
      throw new Error("请在 Auto Research 桌面 App 中选择保存位置。");
    }
  ...
  async function waitForJob(jobId) {
    for (let attempt = 0; attempt < 600; attempt += 1) {
      ...
      packageCenter?.rememberJob(job);
      setPackageNote(`${packageCenter?.stageLabel(job.stage) || "正在安全导入"} · ${job.progress}%`);
      if (job.terminal) {
        if (job.stage === "failed") {
