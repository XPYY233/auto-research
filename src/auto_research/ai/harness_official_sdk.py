    ...
    *,
    ...
    if scope == "librarian":
        ...
        return (
            ...
            "引用跨 evidence bundle 时，每个带单位数值只能作为单一 R 引用的逐条来源观察；"
            "跨来源归纳句只能写定性差异，不得同时列出两个数值，也不得声称数值高低、增减、"
            "差值、比例或范围。温度、能量、剂量等实验条件应单列为适用条件，不要把条件数值"
            "写进材料优劣或结果高低的比较句。"
            "数值表达必须可逐条核验：每个数值直接紧邻自己的单位，不要使用共享末尾单位的"
            "数值列表，不要在数值和单位之间插入 Markdown 标记。原样保留材料牌号；"
            "表号、图号和页码请通过对应 [R编号] 来源卡片定位，不在回答中另列无单位数字。"
            "这些规则同样适用于 direct_conclusion、database_gaps 和 suggested_followups；"
            "输入没有明确给出的数值不写，改为说明缺少哪项证据，不计算新差值或比值。"
        )
    return (
        "现在必须结束工具循环：本轮不得再调用任何工具，只输出一个严格 JSON 对象，"
...
def _system_prompt(scope: str) -> str:
    ...
    if scope == "librarian":
        return common + (
            ...
            "每个带单位数值只能作为单一 R 引用的逐条来源观察；跨来源归纳句不得同时列出两个"
            "数值，也不得声称数值高低、增减或排序。温度、能量、剂量等实验条件应与材料差异"
            "结论分句陈述。"
