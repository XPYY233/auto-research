...
> 后续修复继续覆盖同一build54，不得产生build55、DMG或UserKit。源码检查点`ad72177`包含工作区表格对同源官方已核验结构的严格只读链接、文献提取任务的零模型收尾与重复收费阻断，以及持久化安全取消链和PDF选择取消的零请求反馈：任务目录无法认证时失败关闭，validated检查点只执行本机发布/索引，同一论文与当前PDF已有未完成任务时不会新建收费任务；运行中的收费调用不会被强杀，成功回执先保存，未知结果不会伪装为取消。当前安装App仍是`b2578a6`，尚未包含这些源码修复，因此仍须与其他剩余Fusion流程聚合后进行一次完整覆盖构建和用户验收，才可封箱正式候选。官方资料包继续使用独立版本`1.1.0`，SHA-256为`909cc7323b8a91e3a238e8f49d03fe62022f5a9d078f2cc6785bb8a0b73a69c4`；真实导入与后端复核为59篇、4,369条、59份PDF和291项视觉资产，不得原地修改。新包sidecar契约已就绪；若分发必须使用独立新包版本并重新签名验收。
