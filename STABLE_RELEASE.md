...
> **1.2稳定声明仍暂停。** 机器上安装的是单一可覆盖候选`1.2.0`/build54（manifest `release_status=candidate`，core commit `505d0c0`）；build24继续作为已验证稳定回退点。四项AI中的图书管理员、选中证据、私人建议及隔离单页PDF提取已经在本轮真实运行；安装App也已完成官方Table 3真实截图、PDF打开和返回来源标签。当前剩余问题是官方包没有这张表经人工核验的结构化行列，不能从截图猜测补齐。

> 后续修复继续覆盖同一build54，不得产生build55、DMG或UserKit。只有结构化表格恢复、完整Fusion流程及最终串行验收全部通过，才封箱正式候选。官方资料包继续使用独立版本`1.1.0`，SHA-256为`909cc7323b8a91e3a238e8f49d03fe62022f5a9d078f2cc6785bb8a0b73a69c4`；真实导入与后端复核为59篇、4,369条、59份PDF和291项视觉资产，不得原地修改。
