...
> **1.2稳定声明仍暂停。** 机器上安装的是单一可覆盖候选`1.2.0`/build54（manifest `release_status=candidate`，core commit `505d0c0`）；build24继续作为已验证稳定回退点。四项AI中的图书管理员、选中证据、私人建议及隔离单页PDF提取已经在本轮真实运行；安装App也已完成官方Table 3真实截图、PDF打开和返回来源标签。Table 3的5×4候选现已绑定同一官方PDF SHA、第5页和审计bbox，并保存在本机加密审校区，但仍待用户逐格确认，且新源码尚未覆盖安装App。

> 后续修复继续覆盖同一build54，不得产生build55、DMG或UserKit。只有安装App真实显示/核验/导出该表、完整Fusion流程及最终串行验收全部通过，才封箱正式候选。官方资料包继续使用独立版本`1.1.0`，SHA-256为`909cc7323b8a91e3a238e8f49d03fe62022f5a9d078f2cc6785bb8a0b73a69c4`；真实导入与后端复核为59篇、4,369条、59份PDF和291项视觉资产，不得原地修改。新包sidecar契约已就绪，但必须等用户核验后再决定新版本资料包。

> Table 3结构化行列源码批次当前串行通过共享1,139项（80项按设计跳过）和macOS 323项；中立契约与固定指纹金向量已经解除签名包运行时对科学证据层的反向依赖。此结论仅表示源码与隔离验收通过，不表示安装App、用户逐格批准或新版官方包已经完成。
