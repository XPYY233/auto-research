...
> 后续修复继续覆盖同一build54，不得产生build55、DMG或UserKit。官方表格批次已经闭合；新发现的工作区表格展示问题和其他剩余Fusion流程必须继续修复并通过最终用户验收后，才封箱正式候选。官方资料包继续使用独立版本`1.1.0`，SHA-256为`909cc7323b8a91e3a238e8f49d03fe62022f5a9d078f2cc6785bb8a0b73a69c4`；真实导入与后端复核为59篇、4,369条、59份PDF和291项视觉资产，不得原地修改。新包sidecar契约已就绪；若分发必须使用独立新包版本并重新签名验收。
