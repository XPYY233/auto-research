...
> **1.2稳定声明仍暂停。** 机器上安装的是`1.2.0`/build53（manifest `release_status=candidate`，core commit `d41f9d1`）。用户实机已确认它仍存在证据AI身份预检、第二编辑组恢复和图书管理员真实回答等P0，不能称稳定版；build24继续作为已验证稳定回退点。

> 当前源码在同一未编号候选上继续修复，单个缺陷不得产生build54、DMG或UserKit。只有四项真实AI、完整Fusion用户流程、全套测试和安装后验收全部通过，才分配一个新build。官方资料包继续使用独立版本`1.1.0`，SHA-256为`909cc7323b8a91e3a238e8f49d03fe62022f5a9d078f2cc6785bb8a0b73a69c4`；隔离复核为59篇、4,369条、59份PDF和291项视觉资产。
