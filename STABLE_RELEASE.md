...
> **1.2稳定声明仍暂停。** 机器上安装的`1.2.0`/build50来自干净提交`45720ce`；共享1109项（另80项明确跳过）、macOS305项、冻结冒烟、签名、DMG和事务式安装均通过。build51源码候选已收口到运行时代码检查点`5c2055f`并通过共享1116项（另80项跳过）、macOS307项、编译/脚本/资源哈希门，但尚未构建或安装。Mac锁屏仍阻断同一制品的真实WebView点击验收，因此二者都不能提前称稳定版。
