...
> **1.2稳定声明仍暂停。** 机器上安装的是单一可覆盖候选`1.2.0`/build54（manifest `release_status=candidate`，core commit `71f5ddf`）；build24继续作为已验证稳定回退点。四项AI中的图书管理员、选中证据、私人建议及隔离单页PDF提取已经在本轮真实运行，官方包后端PDF也已打开，但Mac锁屏仍阻断安装WebView的官方PDF点击/返回链，官方Table 3真实表格/原图显示也尚未修复。

> 后续修复继续覆盖同一build54，不得产生build55、DMG或UserKit。只有剩余官方PDF WebView链、表图资产显示、完整Fusion流程及最终串行验收全部通过，才封箱正式候选。官方资料包继续使用独立版本`1.1.0`，SHA-256为`909cc7323b8a91e3a238e8f49d03fe62022f5a9d078f2cc6785bb8a0b73a69c4`；真实导入与后端复核为59篇、4,369条、59份PDF和291项视觉资产。
