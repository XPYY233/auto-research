...
class DesktopVersionContractTests(unittest.TestCase):
    def test_launcher_uses_release_contract_generated_version_json(self) -> None:
        ...
        self.assertTrue(metadata["build_number"].isdecimal())
        self.assertGreater(int(metadata["build_number"]), 0)
    ...
    def test_build_manifest_records_the_distribution_boundary(self) -> None:
        ...
        self.assertEqual(manifest["release_channel"], "research-group-candidate")
        ...
        self.assertIn("release candidate", manifest["publication_note"])

    def test_build_manifest_only_calls_an_explicitly_accepted_build_stable(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            project_root = Path(temp_dir)
            desktop_root = project_root / "desktop" / "macos"
            desktop_root.mkdir(parents=True)
