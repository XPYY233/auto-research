...
- 当前发行身份：`1.1.0`（bundle `1.1.0`，build `24`）
...
源码中的 `version.json` 和发布契约已冻结为 `1.1.0` / build `24`。App、DMG、签名和实机验收结果应以本次从干净提交串行构建生成的制品清单为准，不沿用历史版本的测试数或哈希。Fusion上下文栏、两个编辑器组和检查器边界可拖动调整，也可用键盘或双击复位。
