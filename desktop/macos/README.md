...
- 当前源码候选：`1.1.0` / build `27`；真实App与付费AI验收完成前不得称为稳定版
...
源码中的 `version.json` 和发布契约已更新为 `1.1.0` / build `27` 候选。build 25 的 DeepSeek 推理探针失败；build 26 修复提供商连接与工具能力后，又在真实 App 暴露“实验预填验证上限”前后端不一致，因此同样不发布。build 27 修复后必须重新完成四项付费AI与发布验收。App、DMG、签名和实机验收结果必须来自同一干净提交，不沿用旧构建的测试数或哈希。Fusion上下文栏、两个编辑器组和检查器可拖至边缘吸附收起，也可用键盘、按钮或命令面板恢复；宽屏单击证据会自然地在右侧打开预览。
