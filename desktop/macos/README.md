...
- 当前单一源码候选：`1.2.0` / build `54`；它只在完整共享/macOS测试通过后分配，真实 App WebView 与四项 AI 验收完成前不得称为稳定版
...
当前版本包含 Fusion 科研工作台、多标签和双编辑器组、四类服务端筛选、DeepSeek Harness 图书管理员、原文临时高亮与返回链、原生资料包导入与回退、官方/私人/全部联合精确搜索、个人 CSV/TSV/XLSX 导入确认门、分阶段文献提取、`dataset-bundle-v1` 数据集导出与 BYOK 设置。图书管理员只查官方资料和用户已发布文献，不读取私人实验。对话历史保存在本机加密状态区；研究记忆与历史分开保存，只有用户逐条确认且具有核验来源的结论才会写入。

build 54 是本轮全部源码修复、完整共享测试和完整 macOS 测试通过后分配的唯一候选编号，不再为单个缺陷递增。它恢复稳定标签内的证据/PDF挂载、宽松工作区、文献与搜索连续操作，以及四项受控AI主链；图书管理员仍只使用一次有界模型调用，本机先完成 Search V2 召回、硬条件分析和引用冻结，模型只整理受控回答。证据矩阵、相关文章身份和跳转信息继续由本机权威重建。活动记录不会展示模型内部思维链、提示词、工具参数、PDF正文、路径或密钥。图书管理员与证据问答仍经过 prepared action、逐次授权、预算和只读 Harness 边界。它必须重新通过同一安装App的真实WebView与四项付费AI验收，才可替代 build 24 稳定回退点。若验收发现问题，只覆盖修复build54，不再递增编号。Fusion 上下文栏、两个编辑器组和检查器可拖至边缘吸附收起，也可用键盘、按钮或命令面板恢复；拖拽过程中不重建编辑器，损坏布局会回到至少一个可见编辑组。
