...
源码中的 `version.json` 和发布契约已更新为 `1.1.0` / build `43` 候选。build 41 已加入会话绑定后台任务、真实 Harness 活动、可跳转结果栏和可靠分隔栏；build 42 采用任务感知的宽松布局，但真实 App 首次业务授权被冻结运行时的类身份误判阻断，未产生模型调用。build 43 改为严格的结构化同意绑定，并在每次冻结候选中离线验证“准备、签发、一次性消费”闭环。Harness 会在授权预算耗尽前保留一次无工具的最终回答调用，最终结果只接受应用核验过的引用和推荐身份，避免模型回显噪声导致整次回答作废。活动记录不会展示模型内部思维链、提示词、工具参数、PDF正文、路径或密钥。图书管理员与证据问答仍经过 prepared action、逐次授权、预算和只读 Harness 工具边界。它必须重新通过真实 App 的四项付费 AI 验收，才可替代 build 24 稳定回退点。App、DMG、签名和实机验收结果必须来自同一干净提交，不沿用旧构建的测试数或哈希。Fusion上下文栏、两个编辑器组和检查器可拖至边缘吸附收起，也可用键盘、按钮或命令面板恢复；拖拽过程中不重建编辑器，损坏布局会回到至少一个可见编辑组。
