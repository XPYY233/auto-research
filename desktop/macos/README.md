...
- 当前源码候选：`1.1.0` / build `39`；真实App与付费AI验收完成前不得称为稳定版
...
源码中的 `version.json` 和发布契约已更新为 `1.1.0` / build `39` 候选。build 37 已真实跑通连接与图书管理员；build 38 将证据问答限制在当前证据四项只读工具后，真实 DeepSeek 又暴露模型回显应用身份字段不稳定。build 39 改为由服务端恢复 prepared action 中的权威身份，模型只生成解释与局限。它仍须重新通过证据问答及其余付费AI发布验收。App、DMG、签名和实机验收结果必须来自同一干净提交，不沿用旧构建的测试数或哈希。Fusion上下文栏、两个编辑器组和检查器可拖至边缘吸附收起，也可用键盘、按钮或命令面板恢复；宽屏单击证据会自然地在右侧打开预览。
