...
本教程适用于 Apple Silicon Mac 上的 Auto Research v1.1.0（build 24）。
...
打开“设置 → 关于”，应显示 \textbf{v1.1.0 / build 24}。应用程序目录中只应保留一个可启动的 \texttt{Auto Research.app}。旧版回退副本由维护流程保存为不可直接启动的 \texttt{.app.rollback}。
...
工作台三处分隔边界都可调整：拖动“上下文侧栏与中央区”“两个编辑组之间”“中央区与右侧检查器”之间的细分隔条即可改变宽度；双击分隔条恢复默认比例。分隔条获得键盘焦点后，可用左右方向键微调，按住 Shift 可加大步长，Home/End 可直接移动到允许范围的两端。栏宽只作为本机外观偏好保存，不包含论文正文、实验数据或 API 密钥。
...
v1.1.0 表示 Mac 产品流程、Harness安全边界和发布制品达到本次课题组稳定门，不表示固定语料全部完成科学人工复核，也不表示 App 已经 Apple 公证或通过 App Store 审核。新版官方包从原 60 篇范围中明确排除 DOI \texttt{10.2172/6065200}，原因是没有取得可核验的真实 PDF；它不会删除用户本地或 Zotero 中的对应条目。软件稳定、资料包完整和科学准确率是三个独立结论。官方资料包仅限课题组内部、非商业、受限分发；Windows 1.1迁移将在 Mac 主流程获得用户认可后另行开始。
