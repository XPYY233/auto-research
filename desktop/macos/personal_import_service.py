...
def _translate_selection(error: PersonalFileSelectionError) -> PersonalImportServiceError:
...
class PersonalImportService:
    ...
    def __init__(
        ...
        *,
        ...
        self.broker = broker or PersonalFileSelectionBroker()
    ...
    def preview(self, selection_id: str) -> PersonalImportPreview:
        with self._lock:
            ...
            try:
                with self.broker.snapshot(selection_id) as selected:
        ...
        with self._lock:
            ...
            try:
                ...
                if not session.file_registered:
                    with self.broker.snapshot(session.selection_id) as selected:
            ...
            session.sample = sample
            session.pending_draft = draft
            self.broker.revoke(session.selection_id)
