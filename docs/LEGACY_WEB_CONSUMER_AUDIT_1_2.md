...
旧页面中的能力现已按用户价值、安全边界和当前替代链路分类，不再作为删除文件时的
模糊阻塞项：

| 历史能力 | 处置 | 当前/未来权威 |
|---|---|---|
| 图书管理员研究简报快照与Markdown | 保留领域服务，普通Fusion暂不暴露旧入口 | 未来若恢复，只能从当前成功且引用已核验的Harness回答生成瞬态签名快照，再走原生保存目标；不得从聊天历史重签或恢复旧DOM |
| 逐行校准、原始值对比、重开决定、批量核验 | 维护者能力继续保留，旧网页入口退役 | 命令行与当前ReviewQueue/不可变审核版本是权威；若进入产品，必须使用公开实体身份和现有审核队列，禁止localStorage保存内部ID |
| evidence audit、实验画像、旧模型运行、质量候选、审核批次Markdown | 维护/科学审计能力继续保留，普通用户界面退役 | `maintenance.py`、质量流水线、审计命令和独立报告负责；不得把PDF路径、内部ID或维护备注投影给普通用户 |
| 维护者手动`save-snapshot` | 明确退役 | 固定路径写数据库和向页面返回路径不符合桌面安全契约；由官方资料包、用户资料包、dataset bundle及CSV/XLSX原子导出替代 |

这些决定只移除第二套网页控制器，不删除仍被命令行、领域服务或科学审计使用的Python
能力。研究简报若重新进入产品，必须作为独立功能批次设计和验收，不能成为本轮清债的
隐藏扩张项。
...
状态（共享/Mac消费者已完成）：`33e9ae5`已把macOS安全更新器改为从
...
服务覆盖。`b25c195`把CSRF令牌轮换、四域prepared-action精确请求体、取消/未知scope
零收费请求，以及20会话/30天的加密历史投影迁到真实Fusion测试；共享和macOS已无
正向`app.js`消费者。`app.js`仍只因Windows冻结静态映射而暂留，物理删除进入Batch E。
