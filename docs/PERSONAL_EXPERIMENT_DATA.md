...
一线实验人员完成测量后，可以导入自己整理的 CSV、TSV 或 XLSX 数据表。软件先展示列名、样例值、数据含义、单位和实验条件，请用户确认后再进入“我的实验”检索。用户不需要先写成论文，也不需要理解 AI 或数据库。趋势图附件属于后续能力；当前共享 UI 不导入趋势图、不自动生成趋势图，也不从曲线读取数据点。

## 首版流程

1. 使用桌面系统文件选择器选择 CSV/TSV/XLSX；界面只接收一次性 `selection_id`，不读取或显示本机路径。核心在私人边界内保留文件名、大小和哈希以复核不可变身份，这些字段不进入 renderer 或搜索结果。
2. 为每个工作表生成导入预览，只展示少量样例值，不在预览阶段写入个人数据库。
3. 用户确认每列角色、物理意义和单位；数值列的单位即使为空或无量纲，也必须明确确认。
4. 用户补充项目、样品、实验批次、方法、条件和备注，并确认哪些列组成一条测量序列。
5. 全部问题消除、草稿保存且用户明确复核后，才生成私有搜索文档。草稿、拒绝项和含义未确认的列不进入搜索。
...
- 核心预览结果可在私人服务边界内包含文件名、大小和内容哈希用于复核，但 renderer 公共投影不得返回或持久化路径、文件哈希、内部数据库 ID、导入 ID 或草稿 ID。
...
确认请求必须携带最新 `expected_revision` 和用户实际复核的 `reviewed_revision`，两者必须与当前草稿版本一致。用户在复核后修改项目、样品、实验、条件、列角色、含义、单位或序列关系时，旧复核立即失效，必须重新保存并复核；后端和 renderer 都不得用陈旧确认覆盖新草稿或既有 confirmed 记录。
...
共享搜索页分为“本地文献工作区”和“离线资料库”。离线资料库可选择“官方文献 / 我的实验 / 全部”，只执行一次后端联合精确查询；`all` 不由前端分别查询并合并。已知样品号、批次号或明确关键词时可直接搜索私人记录。

Librarian V3 仍固定只查官方文献全库，当前不读取、不比较也不综合私人实验数据。涉及“我的实验”或“全部”的比较、解释和趋势问题必须留在精确搜索，直到未来另行设计明确授权、来源分隔和定量兼容门。

个人实验的公开搜索投影统一使用 `item/table/figure/finding` 四类 `entity_type`，并携带 `source_scope=private`、稳定 `source_id` 和不可反推内部主键的 `entity_uid`。`record_type=experiment_run` 只属于私人仓库内部聚合契约，不会成为第五类搜索结果，私人数据也不写入论文六列事实表。renderer 只消费去路径、去哈希、去内部 ID 的公开投影，并以“我的实验 · 本机私人”明确标记来源。

## 当前尚未实现

- 尚未支持旧二进制 `.xls`、宏工作簿、外部链接或需要执行公式才能得到结果的表格；这些输入会安全拒绝，而不是降级猜测。
- 平台中立仓库、四类只读适配器、官方+私人联合召回、确认门和共享 UI 已进入从 `9985386` 构建的 macOS `0.5.0-preview.1` App/DMG，并通过冻结 smoke 与隔离 GUI 启动验证。
- Windows 后端 parity 已在 `a134acd` 接入共享 HTTP bridge、原生选择、私人导入和联合搜索；仍无 Setup、无 Windows 11 真机验收，`installer_ready=false`。
- 尚未提供仪器原始格式导入、趋势图附件、多人协作或云同步；单机草稿能检测版本冲突，但 UI 不提供自动冲突合并。
