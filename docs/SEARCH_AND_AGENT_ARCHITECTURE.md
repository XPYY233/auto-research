...
- Librarian V3 由本地确定性控制流直接执行 Search V2 四类覆盖召回；检索参数在本地校验，总候选和四类候选分别设上限。`ToolRegistry` 仅保留为旧 Agent 目录兼容面，不执行当前召回，也不允许模型控制检索循环。
- Agent 的 `[R编号]` 必须对应实际返回结果。DeepSeek 失败时精确检索继续工作。
- 图书管理员固定使用官方文献全库；论文范围控件和私人/全部来源控件只属于精确检索。研究问题执行当前官方证据召回，系统能力、普通对话和必须先澄清的请求按本地意图保持零召回；历史回答不能替代新检索。
...
- `workspace`：本地文献工作区，保留 Search V2、论文范围、筛选、导出和只查官方文献全库的 Librarian。
- `offline`：离线资料库，只做平台中立精确检索；来源可选 `official`、`private` 或 `all`。一次用户查询只调用一次联合搜索，`all` 通过省略 `source_scope` 表示，前端不得分别查询后合并或重新排序。

底层路由仍把用户选择拆成两个互不混淆的维度：
...
`search-route-v1` 只做本地确定性分诊，不打开数据库、不重建索引、不调用模型。体验模式为 `exact`、`librarian`、`browse` 和 `filter`。带 DOI、样品号、批次号或明确定位语言的问题优先精确搜索。涉及私人实验或 `all` 的查询当前只能进入精确联合搜索；解释、比较、趋势和综合问题只有在官方文献范围内才能交给 Librarian。DeepSeek 不可用时精确搜索仍须独立工作。
...
所有进入 renderer 的联合搜索 DTO 都经过中央公开投影门：只保留四类展示字段和稳定来源身份，拒绝本机路径、文件/SQLite URI、哈希、内部数据库 ID、导入/草稿 ID 和底层异常。私人卡片只显示项目、样品、材料、方法与条件；官方卡片显示论文、DOI 和页码。没有二进制载荷时，PDF/图片按钮必须禁用而不是生成假跳转。
...
2. 优先复用已有本地意图路由、确定性召回、四类公开 DTO 和综合门；不为新 Agent 恢复模型驱动的 `ToolRegistry` 调用循环。
