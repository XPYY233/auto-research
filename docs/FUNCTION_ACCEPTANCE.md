...
| 旧库迁移与恢复 | workspace_migration、workspace_switch；哈希/记录/锁/进程中断/回退回归 | 真实副本 21 表、550 引用验证；27 个旧哈希类型校正留档 | 日常目录实际切换及重启后联合核验 |
...
| 审核、原子发布与索引 | review_queue、literature_extraction_finalizer、search_index；真实数据库/HTTP 回归 | 安装版队列 160 项；新文章 Figure 1/5 实际显示，5 张均经服务核验 | 人工审核保存、发布、目录更新的完整实机流程 |
| 检索、详情、原图、PDF 定位 | search_index、workspace_evidence_resolver、source_highlight；公开接口回归 | 迁移副本 64 论文/65 文档/309 图像引用全部在新目录且哈希正确 | 安装版逐类详情、定位与重启联合操作 |
