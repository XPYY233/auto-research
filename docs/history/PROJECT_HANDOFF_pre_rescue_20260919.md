...
本次恢复：用户将主账户最低剩余额度改为50%；恢复时读取剩余70%。root与现有前端任务继续串行收口验收，Windows冻结。不降低稳定门、不新增build；以下70%保留线和停手记录属于上一轮历史。

当前安装和逐项结果只以`docs/BUILD54_INSTALLED_ACCEPTANCE.md`顶部为准：同一build54候选已聚合到7ed286b；完整中文图书管理员实机1次/约5秒成功，16引用重启可恢复，R16可打开原图与核验网格；4689条训练数据集已实际保存，JSONL/Parquet逐条一致，重启收据存在。续轮root已实际拖拽上下文栏与两个编辑组收起并恢复，无整页空白；键盘/窄窗口矩阵仍未完。提取真实SQLite幂等收尾、两类用户包真实文件+加密历史+完整服务重启分别32/38目标通过，不等于安装版全流程。前端任务仅修separator焦点，root继续提取/分享验收；Windows冻结。额度最新66%，保留50%；不新建build/DMG/UserKit，仍不是稳定版。
