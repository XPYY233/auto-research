> Historical checkpoints only; use PROJECT_HANDOFF.md for current authority.
...
- Official Table 3 state: the active official package, exact PDF SHA, source page 5 and audited bbox bind one encrypted local `verified` 5×4 structure. CSV/XLSX each reproduce all 20 cells. The package-sidecar signing/audit and old-package compatibility contracts pass; the immutable `1.1.0` package itself is unchanged.
- Source checkpoint: `e72fffe` contains the strict workspace-source table link from `7180df7` plus zero-model extraction finalization and a duplicate-paid-task guard. Exact DOI/PDF SHA/page/screenshot SHA identity may expose an official verified grid read-only while preserving the workspace screenshot. Validated checkpoints finalize locally; unreadable/corrupt task directories fail closed; an unfinished authenticated checkpoint for the same paper/current PDF blocks a new prepared action before model use. These source fixes are not yet in the installed build54 App.
- Remaining ordered gates: keep the same build54, finish the remaining installed workflow and release gates, then perform one aggregated overwrite build and user acceptance. If the verified sidecar is distributed, issue and verify a new independently versioned official package rather than rewriting `1.1.0`.
...
- Three superseded build54 rollback copies, two old release Apps and the clean build54 worktree were removed after identity/hash verification; only the 0.5 comparison, build24 stable rollback and current `b2578a6` candidate rollback remain. The cleanup released about 2.7GB. The unknown-database build49 worktree remains protected. Windows remains frozen.
