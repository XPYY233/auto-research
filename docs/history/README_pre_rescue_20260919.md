...
Last built desktop checkpoint: **Auto Research `0.6.0-preview.1` for Apple Silicon macOS**, artifact-tagged `evidence-demo-2026-08-09-macos-workbench-preview-6` from `7909100`. It is an internal development preview, not a portable public release: the editable local-literature workspace still depends on this checkout and the schema-v12 database. The clean release worktree passed 745 automated tests (479 shared core, 157 macOS and 109 Windows contracts), frozen App smoke, ad-hoc signing, DMG verification and installed GUI inspection. The DMG SHA-256 is `57f782897fdd84262c1e1bf5553c7b0e81efaed707d064ebbfa8fa5aff039022`.

The built candidate contains Librarian V3 shared-UI transport, official/private/all federated exact search, the reviewed private-table import gate and the shared offline/private workflow. Windows backend parity remains source-only with `installer_ready=false`: there is no Setup and no Windows 11 clean-machine acceptance. Platform source readiness must not be reported as a Windows desktop release.
...
Private CSV/TSV/XLSX import uses the desktop native picker, bounded DeepSeek suggestions and one visible review action. Users browse the AI-prefilled project/sample/run and column metadata, correct only mistakes, then click once to import. The backend still enforces `previewed → draft_saved → confirmed/indexable`, revision/CAS and search refresh; AI never confirms data. Renderer-facing cards and errors contain no local path, file hash, internal database ID, import ID, or draft ID. Trend-image attachment is a future capability, not a current UI feature.
