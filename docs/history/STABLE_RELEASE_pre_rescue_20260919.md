...
`/Applications/Auto Research.app`当前为`1.2.0/build54`、`7ed286b`、`candidate`，**不可稳定发布**。完整图书管理员问题已在安装App中1次/约5秒成功，16引用和回答重启恢复；实际训练数据ZIP的4689条JSONL/Parquet逐条一致，回执重启保留。共享1372通过/80跳过、Mac338通过及签名安装门均通过，但提取全链、完整拖拽布局矩阵及原生用户包往返尚未全部验收。续轮已实际拖拽收起/恢复左栏和两编辑组，并补真实SQLite收尾及用户包加密历史往返集成测试；这些不能冒充完整实机通过。当前唯一分项权威为[`BUILD54_INSTALLED_ACCEPTANCE.md`](docs/BUILD54_INSTALLED_ACCEPTANCE.md)。主额度最新66%，用户新保留线50%；不生成build55/DMG/UserKit或未经实机核验的稳定声明。

以下为9月2日及更早检查点；其中安装身份与“已通过”不能覆盖上面的当前失败记录。

> **1.2稳定声明仍暂停。** 机器上安装的是单一可覆盖候选`1.2.0`/build54（manifest `release_status=candidate`，core commit `b2578a6`）；build24继续作为已验证稳定回退点。四项AI中的图书管理员、选中证据、私人建议及隔离单页PDF提取已经在本轮真实运行；安装App已完成官方Table 3真实截图、PDF打开/返回、5×4主栏审核、重启持久化及CSV/XLSX真实导出。可执行文件SHA-256为`1f0e7e7d334248049239cd57aa0628d559489eaa00d089d284c0c26576c8cbd7`。

> 后续修复继续覆盖同一build54，不得产生build55、DMG或UserKit。源码检查点`fab4c8a`包含工作区表格对同源官方已核验结构的严格只读链接、文献提取任务的零模型收尾与重复收费阻断、持久化安全取消链和PDF选择取消的零请求反馈，以及图书管理员返回表格的opaque详情链：任务目录无法认证时失败关闭，validated检查点只执行本机发布/索引，同一论文与当前PDF已有未完成任务时不会新建收费任务；运行中的收费调用不会被强杀，成功回执先保存，未知结果不会伪装为取消；本机表格详情只通过公开身份读取真实截图、可核验二维行列和PDF，缺少行列时不伪造或回退内部数字ID。当前安装App仍是`b2578a6`，尚未包含这些源码修复，因此仍须与其他剩余Fusion流程聚合后进行一次完整覆盖构建和用户验收，才可封箱正式候选。官方资料包继续使用独立版本`1.1.0`，SHA-256为`909cc7323b8a91e3a238e8f49d03fe62022f5a9d078f2cc6785bb8a0b73a69c4`；真实导入与后端复核为59篇、4,369条、59份PDF和291项视觉资产，不得原地修改。新包sidecar契约已就绪；若分发必须使用独立新包版本并重新签名验收。

> Table 3结构化行列源码批次串行通过共享1,139项（80项按设计跳过）和macOS 323项；中立契约与固定指纹金向量已经解除签名包运行时对科学证据层的反向依赖。安装App的审核、重启和导出也已完成，但这不等于新版官方包已经制作或整个1.2产品已经稳定。

> `0.5.1`/build6继续作为已验证工作流和交互从容度的比较基线；当前安全、原子发布、官方包v2和数据集能力必须保留，不能直接回退旧收费路由或旧DOM。以下旧“稳定交付”段落仅是历史证据，不代表当前1.2可发布。

> 当前可验证回退基线仍为 Auto Research `1.1.0` / build `24`。最终单一1.2候选完成干净构建、事务式安装、真实WebView及四项AI验收前不覆盖该结论。Windows 1.2迁移冻结。

> 以下旧build章节只保存历史证据，不构成当前发布指令。过去逐缺陷顺延build的做法已经废止。
