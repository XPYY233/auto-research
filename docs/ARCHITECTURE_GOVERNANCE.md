...
- 当前Harness图书管理员只读官方资料与本机已发布文献；有限历史及用户已确认研究记忆须经准确的版本化披露。私人实验仅进入联合精确搜索，不能因为选择了私人来源就发送给文献AI。
...
2026-09-04当前安装验收权威：`docs/BUILD54_INSTALLED_ACCEPTANCE.md`。
当前安装身份与逐项结果只在该验收表维护；源码HEAD不等于安装版本。
未通过的真实流程优先于旧检查点的调用成功/源码全测记录，不得据后者发布或分配新build。
...
“一次候选构建”是发布批次级门槛，不是缺陷级动作。源码修复期间版本号和 build 保持冻结；任何一项安装后用户路径未通过时，继续在同一源码候选上修复并覆盖临时输出，不积累新编号制品。正式 build 只在完整用户验收表通过后分配。仓库外最多保留一个已验证完整 Git bundle、三个有明确用途的 App 回退点和不可重建的数据/签名输入；其余失败候选与重复制品必须在核验后清理。

当前已验证稳定回退制品是 macOS `1.1.0` build24；`1.2.0` build54仍为单一候选，安装身份、源码与实机差距只以`docs/BUILD54_INSTALLED_ACCEPTANCE.md`为准。源码小改不得顺延build；只有同一聚合候选完成全部安装后用户验收才可分配正式build。旧0.6/0.7记录只存在于历史日志，不再构成回退或发布指令。Windows 1.2迁移继续冻结，直到Mac最终候选完成用户验收。
