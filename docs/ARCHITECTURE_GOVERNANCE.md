...
`9f77aff/build54`仍为候选，已确认的AI内容质量与真实数据集计划失败
优先于旧检查点的调用成功/源码全测记录。不得据后者发布或分配新build。
...
当前已验证稳定回退制品是 macOS `1.1.0` build24；安装中的`1.2.0` build54仍是候选，尚不包含源码检查点`ad72177`的零模型收尾、严格同源表格链接和安全取消链。源码小改不得顺延build；只有同一聚合候选完成全部安装后用户验收才可分配正式build。旧0.6/0.7记录只存在于历史日志，不再构成回退或发布指令。Windows 1.2迁移继续冻结，直到Mac最终候选完成用户验收。
