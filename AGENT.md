...
- Current release line: Apple Silicon macOS `1.2.0`. The installed App is the single overwritable build 54 candidate (`release_status=candidate`, build manifest core commit `505d0c0`); build 24 remains the verified stable rollback. Shared 1,133 tests (80 explicit skips), macOS 309 tests, the installed-App AI flows, isolated one-page extraction, and the official `search → Table 3 source image → PDF → return` chain have passed. The remaining Table 3 issue is missing human-verified row/column structure in the immutable official package, not missing source imagery. Do not create build 55 or claim stability until the structured-table recovery and remaining installed-App gates pass.
