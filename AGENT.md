...
- Current release line: Apple Silicon macOS `1.2.0`. The installed App is the single overwritable build 54 candidate (`release_status=candidate`, build manifest core commit `71f5ddf`); build 24 remains the verified stable rollback. Shared 1,127 tests (80 explicit skips), macOS 308 tests, the installed-App Librarian/selected-evidence/personal-suggestion flows, one isolated one-page PDF extraction, and the official-package backend PDF lease have passed. The installed WebView official `search → evidence → PDF → return` click chain is still blocked by the locked Mac, and the official table/figure asset display issue remains queued behind it. Do not create build 55 or claim stability until those remaining installed-App gates pass.
