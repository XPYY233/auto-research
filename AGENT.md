...
- Current release line: Apple Silicon macOS `1.2.0`. The installed App is the single overwritable build 54 candidate (`release_status=candidate`, build manifest core commit `b2578a6`); build 24 remains the verified stable rollback. Shared 1,139 tests (80 explicit skips), macOS 323 tests, the installed-App AI flows, isolated one-page extraction, official source image/PDF return, and the Table 3 5×4 review/restart/CSV/XLSX flow have passed. Source checkpoint `e72fffe` additionally contains the strict workspace-to-official verified-grid link and a zero-model extraction-finalization recovery path that fails closed on unreadable checkpoints and blocks duplicate paid preparation for the same paper/current PDF. These source fixes are not yet installed. The official table review state is independently encrypted and verified-only; the immutable `1.1.0` package was not rewritten. Do not create build 55 or claim stability until the remaining installed-App gates pass. A package carrying the verified sidecar must use a new independently signed package version.
