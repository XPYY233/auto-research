...
- Current desktop checkpoint: Apple Silicon macOS `0.5.0-preview.1`, artifact source `9985386`, tag `evidence-demo-2026-08-09-macos-workbench-preview-4`. It remains an internal development preview whose editable literature workspace depends on the checkout schema-v12 database, not a portable stable release. The clean release worktree passed 722 tests (467 core / 151 macOS / 104 Windows contracts); the App/DMG passed frozen smoke, ad-hoc signing, image verification and isolated GUI process launch/clean shutdown. Final DMG SHA-256: `cad6c8b8f2769b9006a0820fabf9ee22f03ca36ae5593f6f2e96adc88f2374a1`.
...
- The only user-facing shape is the personal desktop workbench. Auto Research `0.5.0-preview.1` is the current Apple Silicon macOS internal development preview and still depends on the checkout schema-v12 workspace; Windows has backend parity but no verified Setup yet. The old `/Users/USER/Zotero/打开本地编辑工作台.command` and `/Users/USER/Zotero/创建导师公网链接.command` names are retained only as migration notices and must not silently start browser services or tunnels.
