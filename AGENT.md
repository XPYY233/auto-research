...
- Current desktop checkpoint: Apple Silicon macOS `0.6.0-preview.1`, artifact source `7909100`, tag `evidence-demo-2026-08-09-macos-workbench-preview-6`. It remains an internal development preview whose editable literature workspace depends on the checkout schema-v12 database, not a portable stable release. The clean release worktree passed 745 tests (479 core / 157 macOS / 109 Windows contracts), plus the final 22-item UI contract check; the App/DMG passed frozen smoke, ad-hoc signing, image verification and installed GUI inspection. Final DMG SHA-256: `57f782897fdd84262c1e1bf5553c7b0e81efaed707d064ebbfa8fa5aff039022`.
- The built candidate includes Librarian V3 shared-UI transport, platform-neutral official/private/all federated exact search, the reviewed private-import confirmation gate and the shared offline/private workbench UI. Keep the `0.4.0-preview.1` App/DMG and tag `evidence-demo-2026-08-01-macos-workbench-preview-3` as the prior rollback artifact.
...
- Windows backend-parity source is synchronized at `7909100`: it consumes the same three-destination shared UI, AI-assisted reviewed-import service and dynamic Credential Manager resolver without copying the prompt or validator. `installer_ready=false`; there is still no Setup and no Windows 11 clean-machine evidence, so the older `3632377` folder remains only a `BUILD KIT READY / SETUP_PRESENT=NO` artifact.
...
- The only user-facing shape is the personal desktop workbench. Its primary destinations are Literature Processing, Search Data and Upload Experiment Data. Literature Processing reuses the existing PDF-upload and current-article review surfaces as two stages; upload must not silently call DeepSeek, and the single paid extraction action remains explicit. Manual entry and revision history are hidden compatibility views, not product navigation. Windows shares the UI contract but has no verified Setup yet. The old `/Users/USER/Zotero/打开本地编辑工作台.command` and `/Users/USER/Zotero/创建导师公网链接.command` names are retained only as migration notices and must not silently start browser services or tunnels.
