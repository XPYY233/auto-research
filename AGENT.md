...
- Current release line: Apple Silicon macOS `1.2.0`. The installed App is build 53 (`release_status=candidate`, build manifest core commit `d41f9d1`); user testing has rejected it because selected-evidence AI, restored secondary-editor state and other audited flows are not yet reliable. Build 24 remains the verified stable rollback. Current source fixes stay on one unnumbered, overwritable candidate until the complete installed-App checklist passes; do not create build 54 for an isolated fix.
