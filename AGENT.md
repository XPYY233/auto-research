...
## Handoff entry point (updated 2026-09-01)
...
- Current release line: Apple Silicon macOS `1.2.0`. The installed App is the single overwritable build 54 candidate (`release_status=candidate`, build manifest core commit `505d0c0`); build 24 remains the verified stable rollback. Shared 1,133 tests (80 explicit skips), macOS 309 tests, the installed-App AI flows, isolated one-page extraction, and the official `search → Table 3 source image → PDF → return` chain have passed. The Table 3 source batch now has a PDF-hash/page/bbox-bound 5×4 manual-review candidate in the encrypted local review store, plus verified-only CSV/XLSX and signed-package sidecar contracts; it is not yet in the installed App and has not been user-approved. Do not create build 55 or claim stability until the same build54 candidate displays and approves that grid, the signed-package migration decision is made, and the remaining installed-App gates pass.
