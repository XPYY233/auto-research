...
## 1.2 单候选恢复线（2026-08-30，当前唯一开发权威）

- 当前唯一可启动App为`/Applications/Auto Research.app`（`1.2.0` / build53，manifest `release_status=candidate`，core commit `d41f9d1`）。用户实机已经证明它不是稳定版：证据AI在公开身份预检处失败，恢复第二编辑组仍可能显示旧空状态，图书管理员真实回答仍未通过最终结构/科学校验。build24继续作为已验证稳定回退点。
- 当前源码检查点为`689c34f`。`2714615`统一Search V2、详情与Harness的工作区公开证据身份，`4d8cf50`删除恢复真实标签时残留的第二编辑组静态空状态；这些只是源码修复，尚未生成新App，不能写成用户问题已经解决。
- 发布流程已冻结为单候选制：源码提交、目标测试或单个缺陷修复不递增build、不生成App/DMG/UserKit/release worktree；四项真实AI、Fusion用户流程、完整串行测试和安装后验收全部通过后，才分配一个新build并生成一次正式候选。
- 仓库外只保留0.5功能比较App、build24稳定回退App、build53当前候选回退App、一个完整Git bundle以及不可重建的资料包/数据库/软著输入。干净build53发布工作树已删除；含未知SQLite改动的build49工作树继续隔离，不得自动清理。

以下build51/build50内容只作历史追溯，不再描述当前安装版或现行发布步骤。
