...
## v1.1.0 build 41 Mac 候选（2026-08-24，当前开发权威）

- 当前安装版为 build 40；build 24 仍是已验证稳定回退点。build 41 保持 `1.1.0` 语义版本，只有干净提交构建、事务式安装、真实 WebView 和四项付费 AI 验收全部通过后，才能替换稳定结论。
...
- build 41 当前串行验证：共享 `926/926`（80项明确外部样本跳过）、macOS `237/237`，最终可靠性目标 `94/94`；Python编译、生产JavaScript语法、发布契约与差异检查通过。生产SQLite SHA-256保持 `18b9a4a3a4cbdf9ffcbe14fa0e4855727a6211c1f61902e3d30324903bc84`，仍是用户未提交现场，不得暂存或进入构建。
- 尚未完成：干净Git提交、App/DMG、事务式安装和真实DeepSeek四业务验收。Windows继续冻结；本候选没有运行Windows测试或构建。
