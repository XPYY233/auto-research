...
## 1.2 单候选恢复线（2026-09-01，当前唯一开发权威）

- 当前唯一可启动App为`/Applications/Auto Research.app`（`1.2.0` / build54，manifest `release_status=candidate`，core commit `505d0c0`）。build24继续作为已验证稳定回退点；build54尚未通过全部产品验收，不能称稳定版。
...
- Table 3结构化行列源码批次已经闭合但尚未进入安装App：从官方包声明的同一PDF SHA、第5页和维护者审计bbox建立5×4人工转录候选，候选保存在App私有AES-GCM审校区，仍为`manual_review`，没有替用户自动批准。只有`verified`版本可导出CSV/XLSX；候选/拒绝/缺失均不能导出。现行真实制包入口已保留并重新绑定可选sidecar到package id/version、entity和本次PDF SHA，旧包无sidecar继续兼容。不可变`1.1.0`包尚未修改；是否发布携带已核验sidecar的新包，必须在用户完成App内逐格确认后决定。
- 本批源码定向门为154项（领域、macOS、Fusion、真实签名包与发布契约）通过；完整串行门为共享1,139项（80项按设计跳过）和macOS 323项。签名资料包与科学证据层共用的来源身份、边界框、行列规范化和内容指纹已抽到仅依赖标准库的中立契约，消除了`product → evidence`反向依赖；固定金向量锁定v1指纹字节，未放宽敏感信息或科学边界。另用真实官方Table 3在隔离审校根完成身份、5×4候选、批准以及CSV/XLSX逐格一致性验收。无可信bbox的旧包会在任何候选写入前失败关闭，界面不显示死路操作。上述仍是源码/隔离证据，不能替代覆盖build54后的真实WebView核验。
- 发布流程保持单候选制：源码提交、目标测试或单个缺陷修复不递增build、不生成DMG/UserKit。只有结构化表格恢复、剩余WebView流程、最终完整回归和用户验收全部通过后，才封箱一个正式候选。
- 仓库外只保留0.5功能比较App、build24稳定回退App、当前候选回退、一个完整Git bundle以及不可重建的资料包/数据库/软著输入；中间候选和重复发布制品在身份核对后删除。含未知SQLite改动的build49工作树继续隔离，不得自动清理。
