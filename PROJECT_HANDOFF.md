...
- 用户已明确接受0.9.1 Fusion工作台作为后续功能重构的唯一界面底座；这不等于第1—5项功能已恢复，也不授权恢复旧0.8页面。Windows继续暂停。
- 首个界面契约已冻结：文献页顶部同时保留“导入 PDF”和“开始自动提取与核验”两个显眼主操作；搜索页顶部固定“精确检索”和“图书管理员（官方全库）”；实验页顶部固定“选择 CSV / TSV / XLSX”。这些入口当前仍由Fusion只读策略禁用，后续业务版本必须在原位接线，不得再把主流程藏到页面底部。
- 中央详情标签随模块明确预留“证据详情 / 图书管理员 / 列与曲线详情 / 任务详情”；动态内容不再在页面尾部重复生成相同入口。三类真实业务接线必须按0.9.2之后的串行版本推进，并保持单一导航所有者、AI授权门与path-free契约。
...
2. 只在Fusion DOM内串行恢复Mac功能；不复活旧页面，不启动Windows迁移。
3. 先保持顶部三类主操作与图书管理员位置稳定，再逐项接入真实后端；每一步都先跑目标测试。
4. 0.9.1仍只能称为GUI体验版；任何按钮在真实服务、错误处理和用户流程验收前不得伪装成功。
