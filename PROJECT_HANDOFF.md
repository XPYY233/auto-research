...
- 当前源码检查点为`c0fb9a1`：资料包/dataset与personal-import已经从Fusion主脚本物理拆分；用户资料包和训练数据集的成功导出会写入独立macOS加密、path-free的30天回执，并在“资料包 → 任务与收据”跨重启恢复。完成文件若因回执存储暂时失败，会显示“恢复完成回执”；该操作只重新记录可信完成结果，不重新选择位置、不重跑导出或dataset构建，重复点击幂等且失败继续提示“文件已导出，请勿重复导出”。Fusion已把取消零请求、三项用户包风险确认和逐篇PDF权限迁到当前控制器测试；私人实验保存后若搜索刷新失败，实验首屏会显示幂等“恢复私人搜索索引”，不会重复确认、导入或调用AI。共享/Mac测试、macOS更新器和静态冒烟已停止正向读取全部四个旧Web控制器；CSRF、四域prepared-action、一次任务级提取授权、后台续跑和加密历史均由真实Fusion/平台服务覆盖。旧JavaScript只因Windows冻结静态清单而保留，历史简报与维护者工具也已分类为独立领域能力，不再阻塞旧页面退役。导入、失败/取消与长任务的独立持久操作历史，以及pending任务跨App重启恢复仍是后续债务。该检查点只通过目标测试和发布哈希校验，尚未全测、构建、安装或真实用户验收，不得称为新稳定版。
