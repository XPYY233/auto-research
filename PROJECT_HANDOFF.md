...
- 当前源码检查点为`9e63195`：资料包/dataset与personal-import已经从Fusion主脚本物理拆分；用户资料包和训练数据集的成功导出会写入独立macOS加密、path-free的30天回执，并在“资料包 → 任务与收据”跨重启恢复。Fusion已把取消零请求、三项用户包风险确认和逐篇PDF权限迁到当前控制器测试；私人实验保存后若搜索刷新失败，实验首屏会显示幂等“恢复私人搜索索引”，不会重复确认、导入或调用AI。共享测试已停止读取旧`desktop_product.js`和旧`package_center.js`，Mac静态冒烟改为真实Fusion资源；旧JavaScript仍因Windows冻结清单而保留。当前只保存成功导出回执；导入、失败、取消、长任务及`receipt_status=pending`的显式重试仍是后续债务。该检查点只通过目标测试和发布哈希校验，尚未全测、构建、安装或真实用户验收，不得称为新稳定版。
