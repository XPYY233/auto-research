...
- 当前唯一可启动App为`/Applications/Auto Research.app`（`1.2.0` / build54，manifest `release_status=candidate`，core commit `71f5ddf`）。build24继续作为已验证稳定回退点；build54尚未通过完整安装后验收，不能称稳定版。
- build54同批次已经完成：真实安装App中的图书管理员、选中证据问答和私人表格AI建议；隔离单页PDF以2次模型调用完成四类候选、质量门、原子发布和索引刷新；共享1,127项（80项按设计跳过）、macOS 308项及Python/JavaScript/发布哈希检查通过。
- 官方`1.1.0`包已经在真实App首次导入和重复导入，活动库保持59篇、4,369条、59份PDF和291项视觉资产；正式服务已从活动包搜索结果打开约9.2MB、`%PDF-1.5`的官方PDF，并拒绝错误来源范围。`71f5ddf`补齐官方集合类型、PDF能力字段和显式`source_scope`，但Mac锁屏仍阻断安装WebView的`搜索 → 证据 → 查看原文 → 缩放/翻页 → 返回`最终点击门。
- 用户新发现的官方Table 3详情缺少真实表格/原图，必须排在上述PDF点击门之后修复；不得用标题、图注或AI语义伪装表格结构，不得借用工作区资产。修复后仍在同一build54候选上覆盖安装，不创建build55。
- 发布流程保持单候选制：源码提交、目标测试或单个缺陷修复不递增build、不生成DMG/UserKit。只有剩余WebView流程、官方表图资产、最终完整回归和用户验收全部通过后，才封箱一个正式候选。
- 本轮已删除可由Git重建的约1.1GB临时build54工作树及约835MB临时安装回退。仓库外继续只保留0.5功能比较App、build24稳定回退App、当前候选回退、一个完整Git bundle以及不可重建的资料包/数据库/软著输入；含未知SQLite改动的build49工作树继续隔离，不得自动清理。
