...
更新：2026-09-22。此文件是当前进度唯一入口；旧检查点移至 docs/history。
...
Windows 冻结；不新增功能、不购买订阅、不默认调用收费模型。用户于 2026-09-22 解除 Codex 人工额度保留限制；这不扩大应用模型收费、购买服务或公开私人数据的授权。

维护 skill 的权威副本为公开仓库 `skills/auto-research-evidence-maintainer/`，本机入口应链接到此副本；旧科研仓库的同名 skill 仅是历史。恢复任务时先读此文件，再核实 GitHub 和安装状态。
...
用户报告的新文章缺图问题已在 build56 自动补全后显示，刷新修复与 build57 重启验收通过，#33 可关闭。原始阻断：后台存在 5 张待审图像和哈希核验不等于用户流程修复；需核验提取发现→AI 自动核验→自动发布→目录/详情显示，AI 核验通过后自动发布，未通过者保留失败原因；禁止补造表格。
...
#33 第三部分（源码，待候选）：新增只补全图表的受控 AI 任务，复用原文快照与断点预算，正文/数值/既有表格版本保持不变；重复完成不重新收费。历史隔离记录保留，目录只统计同一图像的最新核验状态。已随 PR37/38 进入 build57，实际文章与重启验证通过。
