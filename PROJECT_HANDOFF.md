...
- 当前源码检查点为`7180df7`，已完成随后排队的“工作区表格有同源核验结构却只显示语义、没有行列”修复。工作区表格仅在DOI、工作区PDF实际SHA、来源页及工作区截图实际SHA与活动官方包唯一匹配时，才只读链接官方已核验结构；Table 3真实快照返回5×4行列并标记`官方资料库已核验 · 同源只读`，仍同时显示工作区自己的原始截图。链接状态不会写回工作区、不会借用近似论文/页码/图片，也不会把官方审核状态伪装成本机审核；CSV/XLSX沿用匹配到的官方稳定身份。该源码批次93项共享目标、51项macOS目标和15项发布契约检查通过，但尚未重建/安装App，因此当前安装build54（`b2578a6`）还不包含这项修复。
...
- 发布流程保持单候选制：源码提交、目标测试或单个缺陷修复不递增build、不生成DMG/UserKit。工作区表格展示源码批次已经收口；继续聚合剩余Fusion流程，只有完整安装后用户验收表通过才覆盖构建并封箱一个候选。
