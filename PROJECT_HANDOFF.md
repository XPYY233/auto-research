...
- 本批源码定向门为154项（领域、macOS、Fusion、真实签名包与发布契约）通过；另用真实官方Table 3在隔离审校根完成身份、5×4候选、批准以及CSV/XLSX逐格一致性验收。无可信bbox的旧包会在任何候选写入前失败关闭，界面不显示死路操作。上述仍是源码/隔离证据，不能替代覆盖build54后的真实WebView核验。
